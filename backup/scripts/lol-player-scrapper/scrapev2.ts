...
async function scrapePlayerLeagueOfGraphs(page: Page, original: string, normalized: string) {
  ...
  try {
    const personalRating = await page.$eval(
      '.mainRankingDescriptionText div.queueLine span.queue',
      (el) => el.textContent?.trim() || 'N/A',
    );
    const rankTier = await page.$eval(
      '#mainContent .summoner-rankings > div.best-league .txt.mainRankingDescriptionText > div.leagueTier',
      (el) => el.textContent?.trim().replace(/\s+/g, ' ') || 'N/A',
    );

    if (personalRating == 'Soloqueue') {
      playerData.career.currentSoloDuoRank = rankTier;
      const flexRating = await page.$eval(
        '#mainContent .summoner-rankings .row.other-league-content-row .queueName.text-left.show-for-light-only',
        (el) => el.textContent?.trim() || 'N/A',
      );
      if (flexRating == 'Draft Ranked Flex') {
        const flexTier = await page.$eval(
          '#mainContent .summoner-rankings .other-league-content-row .leagueTier',
          (el) => el.textContent?.trim().replace(/\s+/g, ' ') || 'N/A',
        );
  ...
  // Avg Enemies Rating
  const avgEnemiesRating = await pageElementToString(
    await page.$(
      '#mainContent .summoner-rankings .other-league.even.averageEnnemyLine .other-league-content .leagueTier.no-margin-bottom',
    ),
  );
  playerData.avgEnemiesRatingInLastTenGames = avgEnemiesRating;

  // Ranking History
  const rankingHistory = await page.$$eval('.tag.requireTooltip.brown', (els) =>
    els.map((e) => {
  ...
  if (rankingHistory.length === 0) {
    ...
    for (let i = 0; i < rankingHistory.length; i++) {
      ...
      playerData.career.soloDuo.push({
        season: rankingHistory[i].season,
        tier: rankingHistory[i].soloDuoPeak,
      });
      playerData.career.flex.push({
        season: rankingHistory[i].season,
        tier: rankingHistory[i].flexPeak,
      });
    }
  }

  const positiveQuirk = await page.$$eval('.tag.requireTooltip.green', (els) =>
    els.map((e) => {
      const tooltip = e.getAttribute('tooltip') || '';
  ...
  const negativeQuirk = await page.$$eval('.tag.requireTooltip.red', (els) =>
    els.map((e) => {
      const tooltip = e.getAttribute('tooltip') || '';
