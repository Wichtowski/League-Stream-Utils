...
import { TeamPasswordModal } from "@libdraft/components/TeamPasswordModal";
...
export default function TeamdraftPage() {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        const response = await authenticatedFetch(`${API_BASE_URL}/draft/sessions/${sessionId}`);
        if (response.ok) {
  ...
  const fetchSession = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        router.push("/modules/draft/static");
      }
    } catch (_error) {
      await showAlert({ type: "error", message: "Failed to load session" });
      router.push("/modules/draft/static");
  ...
  const handleReady = async () => {
    ...
    try {
      const response = await authenticatedFetch(`${API_BASE_URL}/draft/sessions/${sessionId}/action`, {
        method: "POST",
        headers: {
  ...
  if (showPasswordModal && !isAuthenticated) {
    return (
      <TeamPasswordModal
        ...
        onCancel={() => router.push("/modules/draft/static")}
      />
    );
  }
