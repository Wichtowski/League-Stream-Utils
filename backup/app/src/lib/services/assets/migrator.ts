...
export class AssetMigrator {
   ...
   */
  static async migrateChampions(expectedChampionKeys: string[], options: MigrationOptions): Promise<MigrationResult> {
    ...
    try {
      ...
      const versionsToCheckFiltered = versionsToCheck.filter((v) => v !== targetVersion).slice(0, maxVersionsToCheck);
   ...
   */
  static async migrateItems(expectedItemKeys: string[], options: MigrationOptions): Promise<MigrationResult> {
   ...
   */
  static async migrateOverlayAssets(expectedAssets: string[], options: MigrationOptions): Promise<MigrationResult> {
    ...
    try {
      ...
      for (const assetKey of expectedAssets) {
        ...
        const filePath = AssetValidator.generateCachedPath(`game/${targetVersion}/overlay/${category}/${filename}`);
