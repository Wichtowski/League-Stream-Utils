...
const TeamDisplay: React.FC<TeamDisplayProps> = ({ team, players, side, teamColor }) => {
  ...
  return (
    ...
    >
      <div className="p-6 h-full flex flex-col"></div>
    </motion.div>
  );
};

export const GameCamera: React.FC<GameCameraProps> = ({ match, blueTeamData, redTeamData }) => {
  if (!gameData || !gameData.allPlayers) {
    return null;
  }

  // Determine which team is on which side based on the current game
  const determineTeamSides = (): {
    blueTeam: Team | null;
    redTeam: Team | null;
    bluePlayers: any[];
    redPlayers: any[];
    ...
    const bluePlayers = bound.blue
      .filter(Boolean)
      .map((bp) => bp.livePlayer)
      .filter(Boolean);
    const redPlayers = bound.red
      .filter(Boolean)
      .map((bp) => bp.livePlayer)
      .filter(Boolean);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none">
      {/* Left Side - Blue Team */}
      <TeamDisplay team={blueTeam} players={bluePlayers} side="left" teamColor="blue" />

      {/* Right Side - Red Team */}
      <TeamDisplay team={redTeam} players={redPlayers} side="right" teamColor="red" />
    </div>
  );
};
