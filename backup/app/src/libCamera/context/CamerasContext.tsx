...
export function CamerasProvider({ children }: { children: ReactNode }) {
    ...
    async (showLoading = true): Promise<Team[]> => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          setError(errorText || `Failed to fetch camera settings (HTTP ${response.status})`);
          return [];
        }
  ...
  useEffect(() => {
    ...
    // Intentionally exclude loadCachedData to avoid effect thrashing
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user]);

  // Background sync interval - only when actively using cameras
  // Disabled periodic sync to avoid spamming API in camera pages
  // useEffect(() => {
  ...
  const updateCameraSettings = useCallback(
    async (settings: { teams: Team[] }): Promise<{ success: boolean; error?: string }> => {
      try {
        if (isLocalDataMode) {
  ...
  const value: CamerasContextType = useMemo(
    ...
    [teams, allPlayers, loading, error, refreshCameras, updateCameraSettings, clearCache]
  );
