...
import type { draftSession, draftConfig, draftAction, LCUStatus } from "@lib/types";
...
interface draftContextType {
  ...
  // Session management
  createSession: (config: draftConfig) => Promise<{ success: boolean; session?: draftSession; error?: string }>;
  ...
  // Pick/ban actions
  performAction: (action: draftAction) => Promise<{ success: boolean; error?: string }>;
...
const SESSIONS_CACHE_KEY = "draft-sessions";
...
export function draftProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSessionsFromAPI = useCallback(
    async (showLoading = true): Promise<draftSession[]> => {
      ...
      try {
        const response = await authenticatedFetch("/api/v1/draft/sessions");
        ...
        console.error("draft sessions fetch error:", err);
        return [];
      } finally {
        if (showLoading) setLoading(false);
      }
    ...
    (message: unknown): void => {
      ...
      if (typeof message === "object" && message !== null) {
        ...
        switch (msg.type) {
          case "session_update":
            if (msg.session) {
              setCurrentSession(msg.session as draftSession);
            }
            break;
          case "action_performed":
            ...
            if (currentSession && msg.action) {
              const updatedSession = {
                ...currentSession,
                actions: [...(currentSession.actions || []), msg.action as draftAction]
              };
  ...
  const initializeWebSocket = useCallback(
    (sessionId: string): void => {
      ...
      const wsUrl = `${protocol}//${window.location.host}/api/v1/draft/ws?sessionId=${sessionId}`;

      wsRef.current = new WebSocket(wsUrl);

      wsRef.current.onopen = () => {
        console.log("WebSocket connected to draft session");
        setConnected(true);
      ...
      wsRef.current.onclose = () => {
        console.log("WebSocket disconnected from draft session");
        setConnected(false);
  ...
  const loadCachedData = useCallback(async (): Promise<void> => {
    ...
    try {
      const [cachedSessions, cachedLcuStatus] = await Promise.all([
        storage.get<draftSession[]>(SESSIONS_CACHE_KEY, { ttl: CACHE_TTL }),
      ...
      // Don't check LCU status on initial load - only when actively using draft
    } catch (err) {
      console.error("Failed to load cached draft data:", err);
  ...
  // LCU status check interval - only when actively using draft
  useEffect(() => {
  ...
  // Initial LCU status check - only when user explicitly accesses draft features
  const initializeLCUCheck = useCallback(async (): Promise<void> => {
    if (!user || authLoading) return;

    // Only check LCU status if we have cached sessions or user is actively using draft
    const cachedSessions = await storage.get<draftSession[]>(SESSIONS_CACHE_KEY, { ttl: CACHE_TTL });
  ...
  const joinSession = useCallback(
    async (sessionId: string): Promise<{ success: boolean; error?: string }> => {
      try {
        const response = await authenticatedFetch(`/api/v1/draft/sessions/${sessionId}`);
  ...
  const startSession = useCallback(
    async (sessionId: string): Promise<{ success: boolean; error?: string }> => {
      try {
        const response = await authenticatedFetch(`/api/v1/draft/sessions/${sessionId}/start`, {
          method: "POST"
        });
  ...
  const endSession = useCallback(
    async (sessionId: string): Promise<{ success: boolean; error?: string }> => {
      try {
        const response = await authenticatedFetch(`/api/v1/draft/sessions/${sessionId}/end`, {
          method: "POST"
        });
  ...
  const performAction = useCallback(
    async (action: draftAction): Promise<{ success: boolean; error?: string }> => {
      if (!currentSession) {
        return { success: false, error: "No active session" };
      }

      try {
        const response = await authenticatedFetch(`/api/v1/draft/sessions/${currentSession._id}/action`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const undoLastAction = useCallback(async (): Promise<{
    ...
    try {
      const response = await authenticatedFetch(`/api/v1/draft/sessions/${currentSession._id}/undo`, {
        method: "POST"
      });
  ...
  const updateSessionConfig = useCallback(
    async (sessionId: string, config: Partial<draftConfig>): Promise<{ success: boolean; error?: string }> => {
      try {
        const response = await authenticatedFetch(`/api/v1/draft/sessions/${sessionId}/config`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ config })
        });
