      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="p-2 rounded mt-1 break-all flex items-center justify-between">
            <Link
              href={`/modules/draft/config/${urls.sessionId}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-400 hover:underline flex-1 mr-2"
            >
              {typeof window !== "undefined" && window.location.origin}
              /modules/draft/config/{urls.sessionId}
            </Link>
            <CopyButton
              text={`${typeof window !== "undefined" ? window.location.origin : ""}/modules/draft/config/${urls.sessionId}`}
              size="sm"
            />
          </div>
        </div>
