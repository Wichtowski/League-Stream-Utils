...
export const SpectatorDisplay: React.FC<SpectatorDisplayProps> = ({ gameData, isConnected, error }) => {
  if (!isConnected) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center">
        <div className="text-white text-xl">{error ? `Error: ${error}` : "Connecting to League Client..."}</div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 text-white font-sans">
      ...
      <div className="absolute top-4 right-4 bg-black/80 text-white p-4 rounded-lg">
        ...
        <div className="space-y-1 text-sm">
          <div>
            <strong>Mode:</strong> {gameData.gameData?.gameMode || "Unknown"}
          </div>
          <div>
            <strong>Map:</strong> {gameData.gameData?.mapName || "Unknown"}
          </div>
          <div>
            <strong>Time:</strong> {Math.floor((gameData.gameData?.gameTime || 0) / 60)}:
            {(gameData.gameData?.gameTime || 0) % 60 < 10 ? "0" : ""}
            {Math.floor((gameData.gameData?.gameTime || 0) % 60)}
          </div>
          <div>
