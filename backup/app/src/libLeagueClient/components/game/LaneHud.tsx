  ...
  return (
    <div>
      {(() => {
        ...
        const renderItems = (
          items: (typeof gameData.allPlayers)[number]["items"],
          ...
          if (resolution === "WQHD" || resolution === "4K") {
            ...
            return (
              <div className="relative">
                <div
                  className={`flex ${align === "left" ? "flex-row-reverse" : "flex-row"} ${resolutionStyles.itemPadding} ${resolutionStyles.spacing} h-full bg-black/85`}
                >
                      ...
                      {align === "left"
                        ? topRowItems.reverse().map((it, idx) => (
                            <div key={idx} className={itemStyle}>
                              {it ? (
                                <Image
                                  src={getItemImage(it.itemID)}
                                  alt={it.name}
                                  fill
                                  sizes="32px"
                                  className={`object-cover ${isDead ? "grayscale brightness-50" : ""}`}
                                />
                              ) : null}
                            </div>
                          ))
                  ...
                  <div className="flex items-center justify-center relative">
                    <div className="flex flex-col items-center">
                      <div
                        className={`text-white ${resolutionStyles.fontSize} mt-[-6px] font-bold rounded min-w-[16px] text-center mb-1 z-30 bg-transpa `}
                      >
                        {fixedWardScore}
                      </div>
                      <div className={`${itemStyle} ${resolutionStyles.trinketSize} mt-[-12px] `}>
                        {gameTrinkets.includes(trinketItem?.itemID || 0) && trinketItem ? (
                          <Image
                            src={getItemImage(trinketItem.itemID)}
                            alt={trinketItem.name}
                            fill
                            sizes="32px"
                            className={`object-cover ${isDead ? "grayscale brightness-50" : ""}`}
                          />
          ...
          return (
            <div className="relative">
              ...
              <div
                className={`absolute -top-0.5 z-30 pointer-events-none text-white ${resolutionStyles.fontSize} font-bold px-2 rounded min-w-[16px] text-center -translate-x-1/2`}
                style={{ left: `${trinketCenterLeftPx}px` }}
              >
                {fixedWardScore}
              </div>
            </div>
        ...
        const renderSpells = (
          spell: (typeof gameData.allPlayers)[number]["summonerSpells"],
          isDead: boolean = false,
          flexDir: "row" | "col" = "col"
        ) => {
          ...
          return (
            <div className={`flex ${flexDir === "row" ? "flex-row" : "flex-col"}`}>
              <div className={`${runeAndSpellStyle} ${bgColor}`}>
                {s1 ? (
                  <Image
                    src={getSummonerSpellImageByName(s1)}
                    alt={s1}
                    width={32}
                    height={32}
                    className={isDead ? "grayscale brightness-50" : ""}
                  />
                ) : null}
              </div>
              <div className={`${runeAndSpellStyle} ${bgColor}`}>
                {s2 ? (
                  <Image
                    src={getSummonerSpellImageByName(s2)}
                    alt={s2}
                    width={32}
                    height={32}
                    className={isDead ? "grayscale brightness-50" : ""}
        ...
        const renderRunes = (
          runes: (typeof gameData.allPlayers)[number]["runes"],
          isDead: boolean = false,
          flexDir: "row" | "col" = "col"
        ) => {
              ...
              <div className={`relative ${resolutionStyles.championSize}`}>
                ...
                {showLevelAnimation ? (
                  <div className="absolute inset-0 bg-black flex items-center justify-center">
                    <div className="text-white text-3xl font-bold animate-pulse">{level || 1}</div>
                  </div>
                ) : (
                  <div
                    className={`absolute bottom-0 ${align === "left" ? "left-[-6px]" : "right-[-6px]"} bg-black/60 text-white text-sm font-bold px-1 ${align === "left" ? "rounded-br" : "rounded-bl"} shadow-lg`}
                  >
                    {level || 1}
                    <div
                      className={`absolute ${align === "left" ? "left-0 top-0" : "right-0 top-0"} w-0 h-0 ${align === "left" ? "border-l-0 border-r-4 border-b-4 border-t-0 border-l-transparent border-r-black/80 border-b-black/80 border-t-transparent" : "border-l-4 border-r-0 border-b-4 border-t-0 border-l-black/80 border-r-transparent border-b-black/80 border-t-transparent"}`}
                    ></div>
                  </div>
                )}
                {isDead && respawnTimer > 0 ? (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div
                      className="text-white text-3xl font-extrabold leading-none text-center"
                      style={{
        ...
        const renderTexts = (
          playerScore: (typeof gameData.allPlayers)[number]["scores"],
          summonerName: string,
          ...
          return (
            <div
              className={`flex direction-${align} justify-between flex-row${align === "left" ? "-reverse" : ""} ${bgColor} ${resolutionStyles.playerTextWidth} h-full ${resolutionStyles.playerTextPadding}`}
            >
              <div
                className={`flex flex-col items-end min-w-10 min-h-10 ${align === "left" ? "pr-0.5 items-end" : "pl-0.5 items-start"} h-full`}
              >
                <div className={`text-orange-200 font-bold ${resolutionStyles.fontSize}`}>
                  {playerScore.creepScore ?? 0}
                </div>
                <div className={`text-white font-bold ${resolutionStyles.fontSize}`}>
              ...
              <div className={`${resolutionStyles.fontSize} flex-shrink-0 h-full flex flex-col justify-between px-1`}>
                <div
                  className={`font-semibold truncate max-w-[100px] ${align === "left" ? "text-left" : "text-right"}`}
                >
                  {summonerName}
                </div>
                <div className="w-full">
                  {healthPercentage > 0 && (
                    <div
                      className={`w-20 h-2 bg-green-900 mb-0.5 ${align === "right" ? "transform scale-x-[-1]" : ""}`}
                    >
                      <div
        ...
        const renderPlayerSide = (
          ...
          return (
            <div
              className={`flex items-center ${borderColor} ${align === "left" ? "justify-end border-r-2" : "justify-start border-l-2"} flex-1`}
            >
              {resolution === "4K" && (
                <>
                  {align === "left" ? (
        ...
        const ChevronLeft = (): React.ReactElement => <MdPlayArrow className="text-blue-500 rotate-180" />;

        const ChevronRight = (): React.ReactElement => <MdPlayArrow className="text-red-500" />;
        ...
        const renderGoldDiff = (
          ...
          return (
            <div
              className={`${resolutionStyles.goldDiffWidth} h-full relative text-center flex items-center justify-center ${resolutionStyles.goldDiffFontSize} font-bold ${bgColor}`}
            >
              {diff > 0 && (
            ...
            {(() => {
              ...
              return laneOrder.map((lane) => {
                ...
                return (
                  <div
                    key={lane}
                    className={`flex items-center ${resolutionStyles.height} border-x-2 border-y-2 border-b-0 ${borderColor} ${lane === "TOP" ? "border-t-2" : ""}`}
                  >
