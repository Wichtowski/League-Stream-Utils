...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  if (body.startDate && typeof body.startDate === "string") {
    {
      body.startDate = new Date(body.startDate as string);
    }
  }
  if (body.endDate && typeof body.endDate === "string") {
    {
      body.endDate = new Date(body.endDate as string);
    }
  }

  const tournament = await updateTournament(db, id, body as Record<string, unknown>);
  if (!tournament) return notFound("Tournament not found");
