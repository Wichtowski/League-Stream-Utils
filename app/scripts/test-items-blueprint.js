...
const {
  itemsBlueprintDownloader,
} = require("../src/lib/services/items-blueprint-downloader");
...
async function testItemsBlueprint() {
  ...
  try {
    ...
    const progress =
      await itemsBlueprintDownloader.getDownloadProgress(version);
    console.log(
      `📊 Download progress: ${progress.downloaded}/${progress.total} (${progress.percentage}%)`,
    );
    ...
    console.log("\n3. Downloading items blueprint for version 15.13.1...");
    await itemsBlueprintDownloader.downloadBlueprint("15.13.1");
    console.log("✅ Items blueprint for 15.13.1 downloaded successfully!");
