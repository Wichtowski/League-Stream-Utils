...
const ICON_URL = (champ: string) =>
  `${DDRAGON_CDN}/${VERSION}/img/champion/${champ}.png`;
const CHAMP_DETAIL_URL = (champ: string, lang: string) =>
  `${DDRAGON_CDN}/${VERSION}/data/${lang}/champion/${champ}.json`;
const SPELL_ICON_URL = (img: string) =>
  `${DDRAGON_CDN}/${VERSION}/img/spell/${img}`;
...
export default function ChampAbilityPage() {
  ...
  const [downloadProgress, setDownloadProgress] =
    useState<ChampionDownloadProgress>({
      current: 0,
      total: 0,
      itemName: "",
      championName: "",
  ...
  const getChampionIconUrl = useCallback(
    async (championKey: string): Promise<string> => {
      if (isElectron) {
        try {
          const cachedUrl = await assetCache.getChampionIcon(
            championKey,
            VERSION,
          );
  ...
  const getAbilityIconUrl = useCallback(
    async (abilityImage: string): Promise<string> => {
      if (isElectron) {
        try {
          const cachedUrl = await assetCache.getAbilityIcon(
            abilityImage,
            VERSION,
          );
          ...
          console.warn(
            `Failed to get cached ability icon for ${abilityImage}:`,
            error,
          );
    ...
    <div className="min-h-screen text-white p-4 flex flex-col items-center justify-center">
      <div className="w-full max-w-7xl p-4 flex flex-col items-center">
          ...
          <div className="flex justify-center mb-6 w-full">
            <div className="bg-gray-800 rounded-lg p-4 max-w-2xl w-full">
              ...
              <div className="mt-3 text-xs text-gray-400">
                Champions are cached locally with images and ability data for
                faster loading.
                <br />
                <strong>Download All:</strong> Downloads all champions for the
                current version.
                <br />
                <strong>Refresh Cache:</strong> Clears cache and re-downloads
                all champions.
              </div>
            </div>
          </div>
        ...
        >
            ...
            <div className="flex flex-col gap-4 w-full">
              {redTeam.map((champ) => (
                <div
                  ...
                  onDoubleClick={() =>
                    setRedTeam(redTeam.filter((c) => c.key !== champ.key))
                  }
                >
          ...
          >
            <div className="font-bold text-xl mb-3 text-blue-300">
              Blue Team
            </div>
            <div className="flex flex-col gap-4 w-full">
              {blueTeam.map((champ) => (
                <div
                  ...
                  onDoubleClick={() =>
                    setBlueTeam(blueTeam.filter((c) => c.key !== champ.key))
                  }
                >
                  <div className="flex flex-col items-center min-w-[100px]">
                    <Image
        ...
        {abilities.spells.map((spell, index) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="text-sm text-center mt-1 font-bold">
                {["Q", "W", "E", "R"][index]}
              </span>
