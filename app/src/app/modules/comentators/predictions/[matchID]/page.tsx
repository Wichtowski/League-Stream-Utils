...
export default function ComentatorPredictionsMatchPage({ params }: ComentatorPredictionsMatchPageProps): React.ReactElement {
  const user = useUser();
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-2">
              {match.tournament ? `Tournament: ${match.tournament}` : 'Quick Match'}
            </p>
          </div>
          <BackButton to="/modules/comentators/predictions" />
        </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center">
                    <span className="text-white text-lg font-bold">{match.teamA.split(' ')[1] || 'A'}</span>
                  </div>
                  <div>
                    <h2 className="text-xl font-semibold text-white">{match.teamA}</h2>
                ...
                <div className="flex items-center space-x-4">
                  <div>
                    <h2 className="text-xl font-semibold text-white text-right">{match.teamB}</h2>
                    <p className="text-gray-400 text-sm text-right">Red Team</p>
                  </div>
                  <div className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center">
                    <span className="text-white text-lg font-bold">{match.teamB.split(' ')[1] || 'B'}</span>
                  </div>
                </div>
              </div>
