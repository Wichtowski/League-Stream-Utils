...
export default function GamePage({
  ...
  const handleChampionAction = (championId: number, action: "pick" | "ban") => {
    ...
    const canAct =
      gameState.currentTurn?.team === teamSide &&
      gameState.currentTurn?.type === action;
    if (!canAct) return;
  ...
  const isChampionDisabled = (championId: number) => {
    ...
    const allPicks = [
      ...gameState.teams.blue.picks,
      ...gameState.teams.red.picks,
    ];
  ...
  const getChampionStatus = (championId: number) => {
    ...
    const bluePick = gameState.teams.blue.picks.find(
      (c) => c.id === championId,
    );
      ...
      actions={
        <div className="flex items-center space-x-6">
          ...
          <div className="text-right">
            ...
            {teamSide && (
              <div
                className={`px-3 py-1 rounded mt-2 inline-block ${
            ...
            <div className="mt-8 text-center">
              ...
              >
                {gameState.currentTurn.team === "blue"
                  ...
                  : "RED TEAM"}{" "}
                -
                {gameState.currentTurn.type === "ban"
                  ? " BAN PHASE"
                  : " PICK PHASE"}
              </div>
        ...
        <div className="bg-black/40 backdrop-blur-lg rounded-3xl p-8 border border-white/10 shadow-2xl">
          ...
          <div className="grid grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 2xl:grid-cols-14 gap-3">
            {filteredChampions.map((champion) => {
              ...
              const hoverClasses = hoverAnimation.getAnimationClasses(
                champion.id,
              );
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    canAct &&
                    currentAction &&
                    handleChampionAction(champion.id, currentAction)
                  }
                  onMouseEnter={() =>
                    currentAction &&
                    handleChampionHover(
                      champion.id,
                      currentAction as "pick" | "ban",
                    )
                  }
                  onMouseLeave={() =>
                    currentAction &&
                    handleChampionHover(null, currentAction as "pick" | "ban")
                  }
                  disabled={disabled}
