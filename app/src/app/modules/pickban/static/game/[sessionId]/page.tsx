...
export default function StaticPickBanGamePage() {
  ...
  const [currentPhase, setCurrentPhase] = useState<
    "ban1" | "pick1" | "ban2" | "pick2" | "finished"
  >("ban1");
  ...
  const [pendingAction, setPendingAction] = useState<"pick" | "ban" | null>(
    null,
  );
  ...
  const getTeamBans = (team: "blue" | "red") => {
    return actions.filter(
      (action) => action.type === "ban" && action.team === team,
    );
  };
  ...
  const hoverAnimation = useChampionHoverAnimation(
    mockChampSelectData,
    turnInfo,
  );
  ...
  const handleChampionAction = async (
    championId: number,
    actionType: "pick" | "ban",
  ) => {
    ...
    try {
      const response = await authenticatedFetch(
        `${API_BASE_URL}/pickban/sessions/${sessionId}/action`,
        {
          method: "POST",
  ...
  const undoLastAction = async () => {
    ...
    try {
      const response = await authenticatedFetch(
        `${API_BASE_URL}/pickban/sessions/${sessionId}/undo`,
        {
          method: "POST",
        },
      );
  ...
  const getPickedChampions = () => {
    return actions
      .filter((action) => action.type === "pick")
      .map((action) => action.championId);
  };

  const getTeamPicks = (team: "blue" | "red") => {
    return actions.filter(
      (action) => action.type === "pick" && action.team === team,
    );
  };
            ...
            <div className="p-6 border-b border-gray-700">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold">
                  Select Champion to {pendingAction === "pick" ? "Pick" : "Ban"}
                </h2>
                <button
                  onClick={() => {
                    setShowChampionSelect(false);
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              <div className="grid grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                {filteredChampions
                  .filter(
                    (champion) =>
                      !getBannedChampions().includes(champion.id) &&
                      !getPickedChampions().includes(champion.id),
                  )
                  .map((champion) => (
                    <button
                      key={champion.id}
                      onClick={() =>
                        handleChampionAction(champion.id, pendingAction!)
                      }
                      onMouseEnter={() =>
                        hoverAnimation.setHoveredChampionId(champion.id)
                      }
                      onMouseLeave={() =>
                        hoverAnimation.setHoveredChampionId(null)
                      }
                      className={`group relative aspect-square bg-gray-700 rounded-lg overflow-hidden border-2 border-gray-600 hover:border-blue-500 transition-colors ${
                        hoverAnimation.hoveredChampionId === champion.id
                          ? hoverAnimation.animationClasses
                          : ""
                      }`}
                    >
                      <Image
