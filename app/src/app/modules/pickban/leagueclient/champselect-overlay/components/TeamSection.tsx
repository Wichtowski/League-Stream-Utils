    ...
    <div className="w-full">
      ...
      <div className="flex flex-row items-end w-full max-h-screen">
        {fullTeam.map((player, index) => (
          <PlayerSlot key={player.cellId} player={player} index={index} teamColor={tournamentData?.blueTeam?.colors?.primary || teamColor} _currentPhase={currentPhase} hoverState={hoverState} />
        ))}
      </div>
    </div>
  );
