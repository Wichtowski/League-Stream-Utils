...
export default function LeagueClientPickBanPage() {
  ...
  const getChampionImage = (championId: number): string | null => {
    ...
    if (ddragonMatch) {
      ...
      if (userDataPathCache) {
        const base = userDataPathCache.replace(/\\/g, "/");
        const rel = `${version}/champion/${key}/square.png`;
        return `file://${base}${rel}`;
      }
      return `${ddragonMatch[1]}/champion/${ddragonMatch[2]}/loading.png`;
    }
    ...
    if (!champSelectSession) {
      return (
        <div className="text-center py-12">
          ...
          <div className="text-sm text-gray-500">Start a game in League of Legends to see live pick & ban data</div>
        </div>
      );
    }
        ...
        <div className="bg-gray-700 rounded-lg p-4">
          ...
          {currentAction && (
            <div className="bg-yellow-600/20 border border-yellow-600 rounded p-2 text-yellow-400 text-sm">
              Player {currentAction.actorCellId + 1} is {currentAction.type === "ban" ? "banning" : "picking"} a
              champion
            </div>
          )}
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-blue-300 text-sm">{getChampionName(player.championId)}</div>
                    </div>
                  </div>
                );
              })}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-red-300 text-sm">{getChampionName(player.championId)}</div>
                    </div>
                  </div>
                );
              })}
