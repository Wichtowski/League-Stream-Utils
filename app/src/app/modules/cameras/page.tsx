...
export default function CamerasPage() {
  ...
  const totalPlayers = mergedTeams.reduce((sum, team) => sum + team.players.main.length + team.players.substitutes.length, 0);
                ...
                <div key={team.id} className="bg-gray-700 rounded-lg p-4">
                  ...
                  {team.players.substitutes.length > 0 && (
                    <div className="text-sm text-gray-400 mb-2">
                      <span className="font-semibold">Subs:</span> {team.players.substitutes.map(p => p.inGameName).join(', ')}
                    </div>
                  )}
                  <div className="mt-2">
