...
export const POST = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    if (tournament.userId !== user.userId && !user.isAdmin) {
      return NextResponse.json(
        { error: "Forbidden: Only tournament organizer can record games" },
        { status: 403 },
      );
    }
    ...
    if (
      !gameResultData.sessionId ||
      !gameResultData.blueTeam ||
      !gameResultData.redTeam
    ) {
      return NextResponse.json(
    ...
    if (!gameResultData.blueTeam.picks || !gameResultData.redTeam.picks) {
      return NextResponse.json(
        { error: "Game must have picks for both teams" },
        { status: 400 },
      );
    }

    if (
      gameResultData.blueTeam.picks.length !== 5 ||
      gameResultData.redTeam.picks.length !== 5
    ) {
      return NextResponse.json(
