...
export const GET = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    const completedMatches = bracket.nodes.filter(
...
export const PUT = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    if (tournament.userId !== user.userId && !user.isAdmin) {
      return NextResponse.json(
        { error: "Forbidden: Only tournament organizer can update matches" },
        { status: 403 },
      );
    }
    ...
    if (
      updateData.winner !== matchNode.team1 &&
      updateData.winner !== matchNode.team2
    ) {
      return NextResponse.json(
    ...
    const loser =
      updateData.winner === matchNode.team1 ? matchNode.team2 : matchNode.team1;
    ...
    const updatedMatch = updatedBracket.nodes.find(
      (n) => n.id === updateData.matchId,
    );
    if (updatedMatch) {
...
export const DELETE = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    if (tournament.userId !== user.userId && !user.isAdmin) {
      return NextResponse.json(
        { error: "Forbidden: Only tournament organizer can delete brackets" },
        { status: 403 },
      );
    }
