...
export const POST = withAuth(async (req: NextRequest, user: JWTPayload) => {
    try {
        ...
        const canRegisterTeam = team.userId === user.userId || 
                               user.isAdmin || 
                               tournament.userId === user.userId;

        if (!canRegisterTeam) {
            return NextResponse.json({ error: 'You can only register your own teams or teams to tournaments you organize' }, { status: 403 });
        }
...
export const DELETE = withAuth(async (req: NextRequest, user: JWTPayload) => {
    try {
        ...
        // 2. Are an admin, OR
        // 3. Are the tournament organizer
        const canUnregisterTeam = team.userId === user.userId || 
                                 user.isAdmin || 
                                 tournament.userId === user.userId;

        if (!canUnregisterTeam) {
            return NextResponse.json({ error: 'You can only unregister your own teams or teams from tournaments you organize' }, { status: 403 });
        }
