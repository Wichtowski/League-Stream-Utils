  ...
  try {
    ...
    if (!canRegisterTeam) {
      return NextResponse.json(
        {
          error:
            "You can only register your own teams or teams to tournaments you organize",
        },
        { status: 403 },
      );
    }
    ...
    const isAdminOrOrganizer =
      user.isAdmin || tournament.userId === user.userId;
    ...
    if (!isAdminOrOrganizer) {
      ...
      if (team.players.main.length !== 5) {
        return NextResponse.json(
          { error: "Team must have a complete 5-player roster to register" },
          { status: 400 },
        );
      }

      // Check if team players are verified (if required)
      const unverifiedPlayers = team.players.main.filter(
    ...
    const updatedTournament = await registerTeamForTournament(
      tournamentId,
      teamId,
      isAdminOrOrganizer,
    );
    ...
    if (!updatedTournament) {
      return NextResponse.json(
        {
          error:
            "Failed to register team. Tournament may be full or registration is closed.",
        },
        { status: 400 },
      );
    }
  ...
  try {
    ...
    if (!canUnregisterTeam) {
      return NextResponse.json(
        {
          error:
            "You can only unregister your own teams or teams from tournaments you organize",
        },
        { status: 403 },
      );
    }

    const updatedTournament = await unregisterTeamFromTournament(
      tournamentId,
      teamId,
    );
    ...
    if (!updatedTournament) {
      return NextResponse.json(
        { error: "Tournament not found or team not registered" },
        { status: 404 },
      );
    }
