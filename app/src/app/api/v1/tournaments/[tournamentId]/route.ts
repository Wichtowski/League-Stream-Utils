...
export const PUT = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    if (
      ...
      if (
        !currentTournament ||
        (currentTournament.userId !== user.userId && !user.isAdmin)
      ) {
        return NextResponse.json(
          { error: "Tournament not found or forbidden" },
          { status: 404 },
        );
      }
      ...
      if (requireDeadline) {
        ...
        if (!registrationDeadline) {
          return NextResponse.json(
            {
              error:
                "Registration deadline is required when deadline is enabled",
            },
            { status: 400 },
          );
        }
    ...
    if (
      ...
      (tournamentData.maxTeams < 2 || tournamentData.maxTeams > 128)
    ) {
      return NextResponse.json(
...
export const PATCH = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    if (
      !status ||
      !["draft", "registration", "ongoing", "completed", "cancelled"].includes(
        status,
      )
    ) {
