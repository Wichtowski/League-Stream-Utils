  ...
  try {
    ...
    if (!category || !filename) {
      return NextResponse.json(
        { success: false, error: "Missing category or filename" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: "Failed to process asset request" },
      { status: 500 },
    );
  }
}
    ...
    switch (action) {
      ...
      case "download-items":
        try {
          ...
          return NextResponse.json(
            { success: false, error: "Failed to download items" },
            { status: 500 },
          );
        }
      ...
      case "check-items-progress":
        try {
          ...
          return NextResponse.json(
            { success: false, error: "Failed to check items progress" },
            { status: 500 },
          );
        }
