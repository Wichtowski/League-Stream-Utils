...
export const GET = withAuth(async (req: NextRequest, user: JWTPayload) => {
  try {
    ...
    const teams = await TeamModel.find({})
      .select(
        "id name tag verified players.main.verified players.substitutes.verified userId createdAt",
      )
      .sort({ createdAt: -1 });
    ...
    const teamsWithStats = teams.map((team: Team) => ({
      ...
      playerStats: {
        mainVerified: team.players.main.filter((p: Player) => p.verified)
          .length,
        mainTotal: team.players.main.length,
        subsVerified: team.players.substitutes.filter((p: Player) => p.verified)
          .length,
