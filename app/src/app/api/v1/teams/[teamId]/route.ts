  ...
  try {
    ...
    if (teamData.players?.main) {
      if (teamData.players.main.length !== 5) {
        return NextResponse.json(
          { error: "Main roster must have exactly 5 players" },
          { status: 400 },
        );
      }

      const roles = teamData.players.main.map((p) => p.role);
      const requiredRoles: PlayerRole[] = [
        "TOP",
        "JUNGLE",
        "MID",
        "ADC",
      ...
      if (!hasAllRoles || !hasUniqueRoles) {
        return NextResponse.json(
          {
            error:
              "Team must have exactly one player per role: TOP, JUNGLE, MID, ADC, SUPPORT",
          },
          { status: 400 },
        );
      }
    ...
    if (teamData.name || teamData.tag) {
      ...
      if (
        !currentTeam ||
        (currentTeam.userId !== user.userId && !user.isAdmin)
      ) {
        return NextResponse.json(
          { error: "Team not found or forbidden" },
          { status: 404 },
        );
      }
      ...
      if (teamData.tag && !availability.tagAvailable) {
        return NextResponse.json(
          { error: "Team tag is already taken" },
          { status: 409 },
        );
      }
    }
