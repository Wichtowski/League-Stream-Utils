...
export class AssetMigrator {
    ...
    static async migrateChampions(
        ...
        const { targetVersion, versionsToCheck = this.DEFAULT_VERSIONS_TO_CHECK, maxVersionsToCheck = 5 } = options;
        const migratedItems: string[] = [];
        const errors: string[] = [];

        try {
            console.log(`Migrating existing champions for version ${targetVersion}...`);
            ...
            for (const championKey of expectedChampionKeys) {
                const squareImagePath = AssetValidator.generateCachedPath(
                    AssetValidator.generateAssetKey('champion', targetVersion, championKey, 'square.png')
                );
                const fileExists = await AssetValidator.checkFileExists(squareImagePath);
            ...
            if (migratedItems.length > 0) {
                console.log(`Found ${migratedItems.length} champions in current version ${targetVersion}`);
                return { migratedItems, totalFound: migratedItems.length, errors };
            }

            // Check previous versions for existing champions
            console.log(`No champions found in version ${targetVersion}, checking previous versions...`);
            ...
            for (const checkVersion of versionsToCheckFiltered) {
                ...
                for (const championKey of expectedChampionKeys) {
                    ...
                    const squareImagePath = AssetValidator.generateCachedPath(
                        AssetValidator.generateAssetKey('champion', checkVersion, championKey, 'square.png')
                    );
                    const fileExists = await AssetValidator.checkFileExists(squareImagePath);
    ...
    static async migrateItems(
        ...
        const { targetVersion, versionsToCheck = this.DEFAULT_VERSIONS_TO_CHECK, maxVersionsToCheck = 3 } = options;
        const migratedItems: string[] = [];
        const errors: string[] = [];
        ...
        try {
            ...
            for (const itemKey of expectedItemKeys) {
                const iconPath = AssetValidator.generateCachedPath(
                    AssetValidator.generateAssetKey('item', targetVersion, itemKey, 'icon.png')
                );
            ...
            if (migratedItems.length === 0) {
                console.log(`No items found in version ${targetVersion}, checking previous versions...`);
                ...
                for (const checkVersion of versionsToCheckFiltered) {
                    ...
                    for (const itemKey of expectedItemKeys) {
                        ...
                        const iconPath = AssetValidator.generateCachedPath(
                            AssetValidator.generateAssetKey('item', checkVersion, itemKey, 'icon.png')
                        );
                        const fileExists = await AssetValidator.checkFileExists(iconPath);
                    ...
                    if (foundInThisVersion > 0) {
                        console.log(`Found ${foundInThisVersion} items in version ${checkVersion}`);
    ...
    static async migrateOverlayAssets(
        ...
        try {
            console.log(`Checking existing overlay assets for version ${targetVersion}...`);
    ...
    static async updateCategoryManifest(
        ...
        try {
            ...
            console.log(`Successfully updated ${category} category manifest with ${migratedItems.length} migrated items`);
        } catch (error) {
    ...
    static async migrateAllAssets(
        ...
        try {
            ...
            const expectedOverlayAssets = [
                'dragonpit/infernal.png', 'dragonpit/ocean.png', 'dragonpit/hextech.png',
                'dragonpit/chemtech.png', 'dragonpit/mountain.png', 'dragonpit/elder.png',
                'dragonpit/cloud.png', 'default/player.png', 'scoreboard/gold.png',
                'scoreboard/grubs.png', 'scoreboard/tower.png', 'atakhan/atakhan_ruinous.png',
                'atakhan/atakhan_voracious.png', 'baronpit/baron.png', 'baronpit/grubs.png',
                'baronpit/herald.png'
            ];
            ...
            if (updateCategoryProgressFn) {
                await Promise.all([
                    champions.migratedItems.length > 0 ? this.updateCategoryManifest(
                        'champions', champions.migratedItems, expectedChampions.length, targetVersion, updateCategoryProgressFn
                    ) : Promise.resolve(),
                    items.migratedItems.length > 0 ? this.updateCategoryManifest(
                        'items', items.migratedItems, expectedItems.length, targetVersion, updateCategoryProgressFn
                    ) : Promise.resolve(),
                    overlayAssets.migratedItems.length > 0 ? this.updateCategoryManifest(
                        'game-ui', overlayAssets.migratedItems, expectedOverlayAssets.length, targetVersion, updateCategoryProgressFn
                    ) : Promise.resolve()
                ]);
            }

            const totalMigrated = champions.totalFound + items.totalFound + overlayAssets.totalFound;
            const totalErrors = champions.errors.length + items.errors.length + overlayAssets.errors.length;
            ...
            return {
                champions: { migratedItems: [], totalFound: 0, errors: [String(error)] },
