...
class RetryLogic {
    ...
    private defaultRetryCondition(error: unknown): boolean {
        ...
        if (error && typeof error === 'object' && 'name' in error) {
            ...
            if (error.name === 'TypeError' && 'message' in error && typeof error.message === 'string' && error.message.includes('fetch')) return true; // Network error
        }
    ...
    async retryNetworkOnly<T>(
        ...
        return this.retry(operation, {
            ...
            retryCondition: (error) => {
                ...
                return !!(error && typeof error === 'object' && 'name' in error && error.name === 'TypeError' &&
