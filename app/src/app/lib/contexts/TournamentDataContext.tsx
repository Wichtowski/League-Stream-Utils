  ...
  const createTournament = useCallback(async (tournamentData: CreateTournamentRequest): Promise<{ success: boolean; tournament?: Tournament; error?: string }> => {
    try {
      ...
      if (response.ok) {
        ...
        return { success: false, error: errorData.error || 'Failed to create tournament' };
      }
    } catch (err) {
  ...
  const updateTournament = useCallback(async (tournamentId: string, updates: Partial<Tournament>): Promise<{ success: boolean; tournament?: Tournament; error?: string }> => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        return { success: false, error: errorData.error || 'Failed to update tournament' };
      }
    } catch (err) {
  ...
  const deleteTournament = useCallback(async (tournamentId: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        return { success: false, error: errorData.error || 'Failed to delete tournament' };
      }
    } catch (err) {
  ...
  const registerTeam = useCallback(async (tournamentId: string, teamId: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        return { success: false, error: errorData.error || 'Failed to register team' };
      }
    } catch (err) {
    ...
    try {
      if (isLocalDataMode) {
        ...
        const updatedTournaments = tournaments.map(t => {
          if (t.id === tournamentId) {
            const registeredTeams = t.registeredTeams || [];
            return { ...t, registeredTeams: registeredTeams.filter(id => id !== teamId) };
          }
          return t;
        });
        setTournaments(updatedTournaments);
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        return { success: false, error: errorData.error || 'Failed to unregister team' };
      }
    } catch (err) {
