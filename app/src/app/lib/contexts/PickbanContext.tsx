...
interface PickbanContextType {
  ...
  // Configuration
  updateSessionConfig: (sessionId: string, config: Partial<PickbanConfig>) => Promise<{ success: boolean; error?: string }>;
...
export function PickbanProvider({ children }: { children: ReactNode }) {
  ...
  const joinSession = useCallback(async (sessionId: string): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  const startSession = useCallback(async (sessionId: string): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  const undoLastAction = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    if (!currentSession) {
      return { success: false, error: 'No active session' };
    }
  ...
  const updateSessionConfig = useCallback(async (sessionId: string, config: Partial<PickbanConfig>): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  return (
    <PickbanContext.Provider value={value}>
      {children}
