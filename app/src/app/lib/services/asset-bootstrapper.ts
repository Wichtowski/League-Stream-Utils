...
import { gameUIBlueprintDownloader } from './game-ui-blueprint-downloader';
import { itemsBlueprintDownloader } from './items-blueprint-downloader';
import { championsBlueprintDownloader } from './champions-blueprint-downloader';
import { runesBlueprintDownloader } from './runes-blueprint-downloader';
...
export const downloadAllAssets = async (
    ...
    const categoryTrackers = new Map<AssetCategory, CategoryTracker>([
        ['champion', { category: 'champion', current: 0, total: 0, stage: 'waiting', currentAsset: 'Waiting...' }],
        ['item', { category: 'item', current: 0, total: 0, stage: 'waiting', currentAsset: 'Waiting...' }],
        ['game-ui', { category: 'game-ui', current: 0, total: 0, stage: 'waiting', currentAsset: 'Waiting...' }],
        ['spell', { category: 'spell', current: 0, total: 0, stage: 'waiting', currentAsset: 'Waiting...' }],
        ['rune', { category: 'rune', current: 0, total: 0, stage: 'waiting', currentAsset: 'Waiting...' }]
    ]);
    ...
    const championTask = async (): Promise<void> => {
        ...
        onProgress?.({ current: 0, total: 0, itemName: 'champions', stage: 'checking', percentage: 0, category: 'champion' });
    ...
    const gameUITask = async (): Promise<void> => {
        ...
        onProgress?.({ current: 0, total: 0, itemName: 'game-ui-assets', stage: 'checking', percentage: 0, category: 'game-ui' });
        ...
        try {
            ...
            onProgress?.({ current: 0, total: 0, itemName: 'game-ui-assets', stage: 'error', percentage: 0, category: 'game-ui' });
        }
    };
    ...
    const spellTask = async (): Promise<void> => {
        ...
        onProgress?.({ current: 0, total: 0, itemName: 'spells', stage: 'complete', percentage: 100, category: 'spell' });
    };
    ...
    const runeTask = async (): Promise<void> => {
        ...
        try {
            ...
            onProgress?.({ current: 0, total: 0, itemName: 'runes', stage: 'error', percentage: 0, category: 'rune' });
        }
    };
