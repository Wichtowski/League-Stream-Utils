...
export interface UltraFastDownloadTask {
    id: string;
...
export interface UltraFastDownloadProgress {
...
export interface UltraFastDownloadStats {
    totalFiles: number;
...
class UltraFastAssetDownloader {
    private isDownloading = false;
    private downloadQueue: UltraFastDownloadTask[] = [];
    private activeDownloads = new Set<string>();
    private maxConcurrentDownloads = 50; // Ultra-aggressive concurrency
    private downloadStats: UltraFastDownloadStats = {
        totalFiles: 0,
        downloadedFiles: 0,
        failedFiles: 0,
        totalSize: 0,
    ...
    private progressCallbacks: ((progress: UltraFastDownloadProgress) => void)[] = [];
    private startTime = 0;
    ...
    onProgress(callback: (progress: UltraFastDownloadProgress) => void): void {
        this.progressCallbacks.push(callback);
    }

    private updateProgress(progress: Partial<UltraFastDownloadProgress>): void {
        const now = Date.now();
        if (now - this.lastProgressUpdate < 25) return; // Ultra-fast updates (25ms)

        this.lastProgressUpdate = now;

        const fullProgress: UltraFastDownloadProgress = {
            stage: 'downloading',
    ...
    async downloadAllAssets(): Promise<{ success: boolean; stats: UltraFastDownloadStats; errors: string[] }> {
    ...
    private async buildDownloadQueueFromIntegrity(integrityResult: AssetIntegrityResult): Promise<void> {
        ...
        const otherAssets = allMissingAssets.filter(asset =>
    ...
    private addAssetTask(assetKey: string, type: 'champion' | 'item' | 'spell' | 'rune' | 'general', priority: number): void {
    ...
    private addChampionTasks(championKey: string): void {
        ...
        const imageSizes = { square: 50000, splash: 200000, splashCentered: 200000, loading: 150000 };
    ...
    private async processDownloadTask(task: UltraFastDownloadTask, worker: { id: string; busy: boolean }): Promise<void> {
    ...
    getDownloadStats(): UltraFastDownloadStats {
...
export const ultraFastAssetDownloader = new UltraFastAssetDownloader(); 