...
export const useTeamLogo = ({ initialLogo, onLogoChange }: UseTeamLogoProps) => {
  ...
  const attemptAutoPreviewUrl = useCallback(async (url: string): Promise<void> => {
    try {
  ...
  const handleLogoUrlChange = useCallback((url: string): void => {
    clearFileOnly();
    setLogoUrlInput(url);
  ...
  const handleLogoFileChange = useCallback(async (file: File): Promise<void> => {
    if (!file) return;
