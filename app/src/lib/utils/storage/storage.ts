...
class UniversalStorage {
  ...
  constructor() {
    ...
    this.useLocalData =
      this.isElectron &&
      localStorage.getItem("electron-use-local-data") === "true";
  ...
  private performInitialCleanup(): void {
    ...
    try {
      ...
      console.log(
        `localStorage usage: ${(usage.used / 1024 / 1024).toFixed(2)}MB`,
      );
  ...
  private getStorageKey(key: string): string {
    const prefix =
      this.isElectron && this.useLocalData ? "electron-local-" : "web-";
    return `${prefix}${key}`;
  }

  private async setLocalStorageWithQuotaHandling(
    key: string,
    data: StorageData,
  ): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "Failed to store data even after aggressive cleanup, trying minimal data",
          );
          const minimalData = {
            data: data.data,
          ...
          try {
            ...
            console.error(
              "Complete localStorage failure, disabling caching for this session",
            );
          }
        }
      } else {
  ...
  private aggressiveCleanup(): void {
    ...
    for (let i = 0; i < itemsToRemove && i < itemSizes.length; i++) {
      try {
        ...
        console.debug(
          "Failed to remove storage item during aggressive cleanup:",
          itemSizes[i].key,
          error,
        );
      }
  ...
  async set<T>(
    ...
    if (
      this.isElectron &&
      this.useLocalData &&
      window.electronAPI?.storage?.set
    ) {
      try {
        ...
        console.debug(
          "Electron storage not available, falling back to localStorage:",
          err,
        );
  ...
  async remove(key: string): Promise<void> {
    ...
    if (
      this.isElectron &&
      this.useLocalData &&
      window.electronAPI?.storage?.remove
    ) {
      try {
  ...
  async getTimestamp(key: string): Promise<number | null> {
    ...
    if (
      ...
      try {
        const electronData = (await window.electronAPI.storage.get(
          storageKey,
        )) as StorageData | null;
