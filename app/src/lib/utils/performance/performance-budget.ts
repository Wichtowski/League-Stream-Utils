...
export const checkPerformanceBudget = (
  ...
  if (metrics.fcp !== null && metrics.fcp !== undefined) {
    const threshold =
      metrics.fcp <= budget.fcp.good
        ? budget.fcp.good
        : budget.fcp.needsImprovement;
    const severity =
      metrics.fcp <= budget.fcp.good
        ? "good"
        : metrics.fcp <= budget.fcp.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (metrics.lcp !== null && metrics.lcp !== undefined) {
    const threshold =
      metrics.lcp <= budget.lcp.good
        ? budget.lcp.good
        : budget.lcp.needsImprovement;
    const severity =
      metrics.lcp <= budget.lcp.good
        ? "good"
        : metrics.lcp <= budget.lcp.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (metrics.fid !== null && metrics.fid !== undefined) {
    const threshold =
      metrics.fid <= budget.fid.good
        ? budget.fid.good
        : budget.fid.needsImprovement;
    const severity =
      metrics.fid <= budget.fid.good
        ? "good"
        : metrics.fid <= budget.fid.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (metrics.cls !== null && metrics.cls !== undefined) {
    const threshold =
      metrics.cls <= budget.cls.good
        ? budget.cls.good
        : budget.cls.needsImprovement;
    const severity =
      metrics.cls <= budget.cls.good
        ? "good"
        : metrics.cls <= budget.cls.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (metrics.ttfb !== null && metrics.ttfb !== undefined) {
    const threshold =
      metrics.ttfb <= budget.ttfb.good
        ? budget.ttfb.good
        : budget.ttfb.needsImprovement;
    const severity =
      metrics.ttfb <= budget.ttfb.good
        ? "good"
        : metrics.ttfb <= budget.ttfb.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (metrics.tti !== null && metrics.tti !== undefined) {
    const threshold =
      metrics.tti <= budget.tti.good
        ? budget.tti.good
        : budget.tti.needsImprovement;
    const severity =
      metrics.tti <= budget.tti.good
        ? "good"
        : metrics.tti <= budget.tti.needsImprovement
          ? "needsImprovement"
          : "poor";
  ...
  if (bundleMetrics) {
    if (bundleMetrics.totalSize !== undefined) {
      const threshold =
        bundleMetrics.totalSize <= budget.bundleSize.total.good
          ? budget.bundleSize.total.good
          : budget.bundleSize.total.needsImprovement;
      const severity =
        bundleMetrics.totalSize <= budget.bundleSize.total.good
          ? "good"
          : bundleMetrics.totalSize <= budget.bundleSize.total.needsImprovement
            ? "needsImprovement"
            : "poor";
    ...
    if (bundleMetrics.jsSize !== undefined) {
      const threshold =
        bundleMetrics.jsSize <= budget.bundleSize.js.good
          ? budget.bundleSize.js.good
          : budget.bundleSize.js.needsImprovement;
      const severity =
        bundleMetrics.jsSize <= budget.bundleSize.js.good
          ? "good"
          : bundleMetrics.jsSize <= budget.bundleSize.js.needsImprovement
            ? "needsImprovement"
            : "poor";
...
export const getPerformanceScore = (
  ...
  const needsImprovementViolations = violations.filter(
    (v) => v.severity === "needsImprovement",
  ).length;

  // Calculate score: 100 - (poor * 20) - (needsImprovement * 10)
  const score = Math.max(
    0,
    100 - poorViolations * 20 - needsImprovementViolations * 10,
  );
