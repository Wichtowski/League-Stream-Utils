...
export class CacheCompletenessChecker {
  ...
  static async checkChampions(
    ...
    try {
      const actualVersion =
        version || (await DataDragonClient.getLatestVersion());

      // Get all champions from DataDragon
      const championsResponse =
        await DataDragonClient.getChampions(actualVersion);
   ...
   */
  static async checkOverlayAssets(
    options: CacheValidationOptions = {},
  ): Promise<CacheCompletenessResult> {
  ...
  static async checkCompletenessFromManifest(
    ...
    loadCategoryManifestFn: (
      category: string,
    ) => Promise<{ completedItems: string[] }>,
  ): Promise<CacheCompletenessResult> {
    try {
      ...
      console.error(
        `Error checking ${category} completeness from manifest:`,
        error,
      );
      return {
        isComplete: false,
  ...
  static async checkAllAssets(options: CacheValidationOptions = {}): Promise<{
    ...
    const totalMissing =
      champions.missingItems.length +
      items.missingItems.length +
      overlayAssets.missingItems.length;
    const totalAssets =
      champions.totalItems + items.totalItems + overlayAssets.totalItems;
