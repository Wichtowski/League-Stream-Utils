...
export class AssetValidator {
    ...
    static validateAssetPath(assetKey: string, expectedPattern?: RegExp): boolean {
        ...
        // Default pattern: cache/game/version/category/...
        const defaultPattern = /^cache\/game\/[\d.]+\/(champions|items|overlay|runes)\/.*$/;
    ...
    static isKeyFile(assetKey: string): boolean {
        ...
        return assetKey.endsWith('/square.png') || // Champion square image
            assetKey.endsWith('/icon.png') || // Item icon
            assetKey.endsWith('/data.json') || // Data files
            assetKey.includes('/overlay/'); // Game UI overlay files
    }
} 