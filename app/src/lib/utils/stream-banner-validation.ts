...
export const validateStreamBannerForm = (formData: TickerFormData): ValidationError[] => {
  ...
  const duplicates = itemTexts.filter((text, index) => 
    text && itemTexts.indexOf(text) !== index
  );
...
export const hasUnsavedChanges = (
  currentData: TickerFormData, 
  originalData: TickerFormData | null
): boolean => {
  if (!originalData) {
    return currentData.title.trim() !== '' || currentData.carouselItems.length > 0;
  }
  
  return JSON.stringify(sanitizeStreamBannerForm(currentData)) !== 
         JSON.stringify(sanitizeStreamBannerForm(originalData));
};
