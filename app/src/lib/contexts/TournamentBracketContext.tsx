...
interface TournamentBracketContextType {
  ...
  getBracket: (
    tournamentId: string,
  ) => Promise<{ success: boolean; bracket?: Bracket; error?: string }>;
  updateBracket: (
    tournamentId: string,
    bracket: Bracket,
...
const TournamentBracketContext = createContext<
  TournamentBracketContextType | undefined
>(undefined);
  ...
  const getBracket = useCallback(
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error ? err.message : "Failed to get bracket";
  ...
  const recordGameResult = useCallback(
    async (
      ...
      try {
        ...
        // Online mode - use API
        const response = await authenticatedFetch(
          `/api/v1/tournaments/${tournamentId}/games`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        const error =
          err instanceof Error ? err.message : "Failed to record game result";
        return { success: false, error };
      }
    },
    [authenticatedFetch, isLocalDataMode],
...
export function useTournamentBracket(): TournamentBracketContextType {
  ...
  if (context === undefined) {
    throw new Error(
      "useTournamentBracket must be used within a TournamentBracketProvider",
    );
  }
  return context;
}
