...
interface PredictionsContextType {
  ...
  submitPrediction: (matchId: string, prediction: string) => Promise<{ success: boolean; prediction?: Prediction; error?: string }>;
  loading: boolean;
}
...
export function PredictionsProvider({ children }: { children: ReactNode }): React.ReactElement {
  ...
  const getPredictions = useCallback(async (matchId: string): Promise<Prediction[]> => {
    setLoading(true);
    try {
  ...
  const submitPrediction = useCallback(async (matchId: string, prediction: string) => {
    setLoading(true);
    try {
      if (isLocalDataMode && typeof window !== "undefined" && window.electronAPI?.storage?.set) {
        ...
        const existing = (await window.electronAPI.storage.get(`predictions-${matchId}`)) as Prediction[] || [];
