...
export function PredictionsProvider({
    ...
    async (matchId: string): Promise<Prediction[]> => {
      ...
      try {
        if (
          ...
          const data = await window.electronAPI.storage.get(
            `predictions-${matchId}`,
          );
          setLoading(false);
    ...
    async (matchId: string, prediction: string) => {
      ...
      try {
        if (
          ...
          const existing =
            ((await window.electronAPI.storage.get(
              `predictions-${matchId}`,
            )) as Prediction[]) || [];
          const filtered = existing.filter(
            (p) => p.commentator !== commentator,
          );
          const updated = [...filtered, newPrediction];
          await window.electronAPI.storage.set(
            `predictions-${matchId}`,
            updated,
          );
          setLoading(false);
  ...
  return (
    <PredictionsContext.Provider
      value={{ getPredictions, submitPrediction, loading }}
    >
      {children}
    </PredictionsContext.Provider>
  );
