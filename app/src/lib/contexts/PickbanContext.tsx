...
interface PickbanContextType {
  ...
  ) => Promise<{ success: boolean; session?: PickbanSession; error?: string }>;
  joinSession: (
    sessionId: string,
...
export function PickbanProvider({ children }: { children: ReactNode }) {
  ...
  // Session state
  const [currentSession, setCurrentSession] = useState<PickbanSession | null>(
    null,
  );
  ...
  const checkLCUStatus = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await authenticatedFetch(
        "/api/v1/pickban/leagueclient/lcu-test",
      );
  ...
  const initializeLCUCheck = useCallback(async (): Promise<void> => {
    ...
    const cachedSessions = await storage.get<PickbanSession[]>(
      SESSIONS_CACHE_KEY,
      { ttl: CACHE_TTL },
    );
  ...
  const endSession = useCallback(
    async (
      ...
      try {
        const response = await authenticatedFetch(
          `/api/v1/pickban/sessions/${sessionId}/end`,
          {
            method: "POST",
          },
        );
  ...
  const performAction = useCallback(
    async (
      ...
      try {
        const response = await authenticatedFetch(
          `/api/v1/pickban/sessions/${currentSession.id}/action`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const undoLastAction = useCallback(async (): Promise<{
    ...
    try {
      const response = await authenticatedFetch(
        `/api/v1/pickban/sessions/${currentSession.id}/undo`,
        {
          method: "POST",
        },
      );
  ...
  const connectToLCU = useCallback(async (): Promise<{
    ...
    try {
      ...
      const error =
        err instanceof Error ? err.message : "Failed to connect to LCU";
      return { success: false, error };
    } finally {
      setLcuLoading(false);
    }
  ...
  const disconnectFromLCU = useCallback(async (): Promise<{
    ...
    try {
      ...
      const error =
        err instanceof Error ? err.message : "Failed to disconnect from LCU";
      return { success: false, error };
    }
  }, []);
  ...
  const syncWithLCU = useCallback(async (): Promise<{
    ...
    try {
      ...
      const error =
        err instanceof Error ? err.message : "Failed to sync with LCU";
      return { success: false, error };
    }
  }, [currentSession, authenticatedFetch]);
  ...
  const clearCache = useCallback(async (): Promise<void> => {
    await Promise.all([
      storage.remove(SESSIONS_CACHE_KEY),
      storage.remove(LCU_STATUS_CACHE_KEY),
    ]);
    setSessions([]);
    setLcuStatus(null);
  }, []);
  ...
  return (
    <PickbanContext.Provider value={value}>{children}</PickbanContext.Provider>
  );
}
