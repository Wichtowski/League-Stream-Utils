...
const TeamsSubscriptionContext = createContext<
  TeamsSubscriptionContextType | undefined
>(undefined);

export function TeamsSubscriptionProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const teamSubscribers = useRef<Map<string, Set<(team: Team | null) => void>>>(
    new Map(),
  );
  const loadingSubscribers = useRef<Set<(loading: boolean) => void>>(new Set());
  const errorSubscribers = useRef<Set<(error: string | null) => void>>(
    new Set(),
  );
  ...
  const subscribeToTeam = useCallback(
    (teamId: string, callback: (team: Team | null) => void) => {
  ...
  const subscribeToLoading = useCallback(
...
export function useTeamsSubscription(): TeamsSubscriptionContextType {
  ...
  if (context === undefined) {
    throw new Error(
      "useTeamsSubscription must be used within a TeamsSubscriptionProvider",
    );
  }
  return context;
}
