...
const TournamentStatsContext = createContext<
  TournamentStatsContextType | undefined
>(undefined);
...
export function TournamentStatsProvider({ children }: { children: ReactNode }) {
  ...
  const getTournamentStats = useCallback(
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error ? err.message : "Failed to get tournament stats";
  ...
  return (
    <TournamentStatsContext.Provider value={value}>
      {children}
    </TournamentStatsContext.Provider>
  );
}
...
export function useTournamentStats(): TournamentStatsContextType {
  ...
  if (context === undefined) {
    throw new Error(
      "useTournamentStats must be used within a TournamentStatsProvider",
    );
  }
  return context;
}
