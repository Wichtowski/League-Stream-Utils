...
export function TeamsProvider({ children }: { children: ReactNode }) {
  ...
  const areTeamsEqual = useCallback(
    (teams1: Team[], teams2: Team[]): boolean => {
  ...
  const fetchTeamsFromAPI = useCallback(
  ...
  useEffect(() => {
    ...
    if (!user || isLocalDataMode || authLoading || !teams || teams.length === 0)
      return;

    // Check if we're on a teams-related page
    const isTeamsPage =
      window.location.pathname.includes("/modules/teams") ||
  ...
  const createTeam = useCallback(
    async (
      ...
      try {
        if (isLocalDataMode) {
          // Create team locally
          const mainPlayers: Team["players"]["main"] =
            teamData.players.main.map((player) => ({
              ...player,
          ...
          const substitutePlayers: Team["players"]["substitutes"] =
            teamData.players.substitutes.map((player) => ({
              ...player,
              id: `local-${Date.now()}-${Math.random()}`,
              verified: false,
              createdAt: new Date(),
          ...
          if (teamData.staff?.coach) {
            staff.coach = {
              ...teamData.staff.coach,
              id: `local-${Date.now()}-coach`,
            };
          }
          if (teamData.staff?.analyst) {
            staff.analyst = {
              ...teamData.staff.analyst,
              id: `local-${Date.now()}-analyst`,
            };
          }
          if (teamData.staff?.manager) {
            staff.manager = {
              ...teamData.staff.manager,
              id: `local-${Date.now()}-manager`,
            };
          }
    ...
    async (
      ...
      try {
        ...
        if (response.ok) {
          ...
          return {
            success: false,
            error: data.error || "Failed to update team",
          };
        }
      } catch (err) {
        const error =
    ...
    async (teamId: string): Promise<{ success: boolean; error?: string }> => {
      try {
        if (isLocalDataMode) {
          ...
          const updatedTeams = teams.map((team) =>
            team.id === teamId
              ? { ...team, verified: true, updatedAt: new Date() }
              : team,
          );
          setTeams(updatedTeams);
          await electronStorage.set(CACHE_KEY, updatedTeams);
        ...
        if (response.ok) {
          ...
          return {
            success: false,
            error: data.error || "Failed to verify team",
          };
        }
      } catch (err) {
        const error =
          err instanceof Error ? err.message : "Failed to verify team";
        return { success: false, error };
      }
    },
    [teams, authenticatedFetch, isLocalDataMode],
  ...
  const verifyPlayer = useCallback(
    async (
      ...
      try {
        if (isLocalDataMode) {
          ...
          const updatedTeams = teams.map((team) => {
            if (team.id === teamId) {
              ...
              const updatedSubPlayers = team.players.substitutes.map(
                (player) =>
                  player.id === playerId
                    ? {
                        ...player,
        ...
        // Online mode - use API
        const response = await authenticatedFetch(
          `/api/v1/teams/${teamId}/players/${playerId}/verify`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (response.ok) {
          ...
          return {
            success: false,
            error: data.error || "Failed to verify player",
          };
        }
      } catch (err) {
        const error =
          err instanceof Error ? err.message : "Failed to verify player";
        return { success: false, error };
      }
    },
    [teams, authenticatedFetch, isLocalDataMode],
  ...
  const verifyAllPlayers = useCallback(
    async (teamId: string): Promise<{ success: boolean; error?: string }> => {
      try {
        if (isLocalDataMode) {
        ...
        // Online mode - use API
        const response = await authenticatedFetch(
          `/api/v1/teams/${teamId}/verify-all`,
          {
            method: "POST",
          },
        );
        ...
        if (response.ok) {
          ...
          return {
            success: false,
            error: data.error || "Failed to verify all players",
          };
        }
      } catch (err) {
        const error =
          err instanceof Error ? err.message : "Failed to verify all players";
        return { success: false, error };
      }
    },
    [teams, authenticatedFetch, isLocalDataMode],
