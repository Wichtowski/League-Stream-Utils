...
export function TeamsProvider({ children }: { children: ReactNode }) {
    ...
    const createTeam = useCallback(
        async (teamData: CreateTeamRequest): Promise<{ success: boolean; team?: Team; error?: string }> => {
            try {
                if (isLocalDataMode) {
                    // Create team locally
                    ...
                    const substitutePlayers: Team['players']['substitutes'] = teamData.players.substitutes.map(
                        (player) => ({
                            ...player,
                            id: `local-${Date.now()}-${Math.random()}`,
                            verified: false,
