...
const TournamentDataContext = createContext<
  TournamentDataContextType | undefined
>(undefined);
...
function stripLogos<
  T extends { logo?: { type?: string; data?: string; url?: string } },
>(tournaments: T[]): T[] {
...
export function TournamentDataProvider({ children }: { children: ReactNode }) {
  ...
  const myTournaments = useMemo(
    () => tournaments.filter((t) => t.userId === user?.id),
    [tournaments, user?.id],
  );
  ...
  const areTournamentsEqual = useCallback(
    (tournaments1: Tournament[], tournaments2: Tournament[]): boolean => {
      ...
      return tournaments1.every((tournament, index) => {
        ...
        return (
          ...
          tournament.registeredTeams?.length ===
            otherTournament.registeredTeams?.length
        );
      });
    },
    [],
  ...
  const fetchTournamentsFromAPI = useCallback(
    async (showLoading = true): Promise<Tournament[]> => {
      ...
      try {
        ...
        const dataChanged = !areTournamentsEqual(
          tournaments,
          fetchedTournaments,
        );
    ...
    try {
      if (isLocalDataMode) {
        ...
        const cachedTournaments =
          await electronStorage.get<Tournament[]>(CACHE_KEY);
  ...
  const unregisterTeam = useCallback(
    async (
      ...
      try {
        ...
        const error =
          err instanceof Error ? err.message : "Failed to unregister team";
        return { success: false, error };
      }
    },
    [authenticatedFetch, isLocalDataMode, tournaments, fetchTournamentsFromAPI],
