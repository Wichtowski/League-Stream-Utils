...
class OBSClient {
    ...
    async getSceneList(): Promise<OBSScene[]> {
        const response = await this.sendRequest('GetSceneList') as { scenes: OBSScene[] };
    ...
    async getCurrentScene(): Promise<string> {
        const response = await this.sendRequest('GetCurrentProgramScene') as { currentProgramSceneName: string };
    ...
    async getStreamingStatus(): Promise<{ isStreaming: boolean; isRecording: boolean }> {
    ...
    // Source Management
    async getSourceList(): Promise<{ sources: Array<{ sourceName: string; sourceType: string }> }> {
        return await this.sendRequest('GetSourceList') as { sources: Array<{ sourceName: string; sourceType: string }> };
    }

    async setSourceEnabled(sourceName: string, enabled: boolean): Promise<void> {
        await this.sendRequest('SetSourceEnabled', { sourceName, sourceEnabled: enabled });
    }

    async getSourceEnabled(sourceName: string): Promise<boolean> {
        const response = await this.sendRequest('GetSourceEnabled', { sourceName }) as { sourceEnabled: boolean };
