...
export class BracketGenerator {
    ...
    static generateBracket(
        ...
        if (settings.type === 'single') {
            return this.generateSingleElimination(tournamentId, seededTeams, bracketSize, settings);
        } else {
            return this.generateDoubleElimination(tournamentId, seededTeams, bracketSize, settings);
        }
    }
     ...
     */
    private static generateWinnerBracket(teams: string[], bracketSize: number): BracketNode[] {
        const nodes: BracketNode[] = [];
        const totalRounds = Math.log2(bracketSize);
     ...
     */
    private static setupSingleEliminationPaths(nodes: BracketNode[], totalRounds: number): void {
        for (let round = 1; round < totalRounds; round++) {
            const currentRoundNodes = nodes.filter(n => n.round === round && n.bracketType === 'winner');
            const nextRoundNodes = nodes.filter(n => n.round === round + 1 && n.bracketType === 'winner');
     ...
     */
    private static seedTeams(teams: string[], seedingMethod: BracketSettings['seeding']): string[] {
    ...
    static getReadyMatches(bracket: BracketStructure): BracketNode[] {
        return bracket.nodes.filter(node =>
            node.status === 'pending' &&
            node.team1 &&
            node.team2
        );
    ...
    static isBracketComplete(bracket: BracketStructure): boolean {
        const finalMatch = bracket.nodes.find(node =>
            node.bracketType === 'grand-final'
        );
