import {
  BaseBlueprintDownloader,
  BlueprintDownloaderConfig,
} from "./base-blueprint-downloader";
...
export class RunesBlueprintDownloader extends BaseBlueprintDownloader<CommunityDragonRuneResponse> {
    ...
    try {
      ...
      const validRunes = allRunes.filter(
        (rune) =>
          rune.iconPath &&
          rune.iconPath !==
            "/lol-game-data/assets/v1/perk-images/Template/7000.png",
      );
      const totalRunes = validRunes.length;

      console.log(
        "Runes data fetched successfully, got",
        totalRunes,
        "valid runes (filtered out template runes)",
      );
      ...
      if (completedRunes.length === 0) {
        const existingRunes = await this.migrateExistingRunes(
          validRunes,
          version,
        );
      ...
      this.updateProgress({
        ...
        currentAsset:
          alreadyDownloaded > 0
            ? `Found ${alreadyDownloaded} runes already downloaded`
            : "Starting rune download...",
        assetType: "rune-images",
      });

      // Download rune icons using the filtered valid runes
      const downloadedCount = await this.downloadRuneIcons(
        validRunes,
        version,
        completedRunes,
      );
      ...
      console.log(
        `${this.config.assetType} blueprint saved to ${blueprintPath}`,
      );
    } catch (error) {
      console.error(
        `Error downloading ${this.config.assetType} blueprint:`,
        error,
      );
  ...
  private async migrateExistingRunes(
    ...
    try {
      for (const rune of validRunes) {
        ...
        let style = rune.key;
        const iconPath = rune.iconPath.replace(
          "/lol-game-data/assets/v1/perk-images/",
          "",
        );
        if (!style) {
  ...
  private async downloadRuneIcons(
    ...
    const runesToDownload = validRunes.filter(
      (rune) => !completedRunes.includes(rune.id.toString()),
    );
    ...
    console.log(
      `Found ${alreadyDownloaded} runes already downloaded, ${runesToDownload.length} runes to download`,
    );
