...
export class ItemsBlueprintDownloader extends BaseBlueprintDownloader<DataDragonItemResponse> {
  ...
  protected progressCallback: ((progress: DownloadProgress) => void) | null =
    null;
  ...
  async downloadBlueprint(version: string): Promise<void> {
    ...
    try {
      ...
      const downloadedCount = await this.downloadItemIcons(
        data,
        version,
        completedItems,
      );
  ...
  private async downloadItemIcons(
    data: DataDragonItemResponse,
    version: string,
  ...
  private async migrateExistingItems(
    data: DataDragonItemResponse,
    version: string,
  ): Promise<string[]> {
    ...
    try {
      ...
      if (existingItems.length > 0) {
        ...
        console.log(
          `Successfully migrated ${existingItems.length} existing items to category manifest`,
        );
      }
