...
class GameUIBlueprintDownloader extends BaseCacheService {
  ...
  private updateGameUIProgress(
    progress: Partial<GameUIDownloadProgress>,
  ): void {
  ...
  async downloadAllGameUIBlueprint(): Promise<GameUIDownloadResult> {
    ...
    try {
      ...
      const totalCount = Object.values(assetCategories).reduce(
      ...
      const categoryProgress = await super.getCategoryProgress(
        "game-ui",
        version,
      );
      ...
      if (processedCount > 0 && errors.length === 0) {
        console.log(
          "All game UI assets processed successfully. Cleaning up manifests for fresh restart capability.",
        );
          ...
          if (typeof window !== "undefined" && window.electronAPI?.isElectron) {
            ...
            try {
              ...
              if (downloadResult.success && downloadResult.localPath) {
                ...
                const sizeResult = await window.electronAPI.getFileSize(
                ...
                totalSize += fileSize;
                console.log(
                  `Successfully processed ${filename}: ${fileSize} bytes`,
                );
              } else {
              ...
              try {
                ...
                const copyResult = await window.electronAPI.copyAssetFile(
                  sourcePath,
                  targetPath,
                );
                ...
                if (copyResult.success && copyResult.localPath) {
                  const sizeResult = await window.electronAPI.getFileSize(
                    copyResult.localPath,
                  );
                  const fileSize = sizeResult.success
                    ? sizeResult.size || 0
                    : 0;
                  ...
                  totalSize += fileSize;
                  console.log(
                    `Successfully copied ${filename} from public folder: ${fileSize} bytes`,
                  );
                } else {
                ...
                throw new Error(
                  `Failed to download asset from local server: ${fileError}`,
                );
              }
            }
          } else {
  ...
  async getOverlayAsset(
    ...
    try {
      ...
      console.error(
        `Failed to get overlay asset ${category}/${filename}:`,
        error,
      );
      return null;
    }
  ...
  private async cleanupManifestAfterSuccess(): Promise<void> {
    try {
      ...
      console.log(
        "Game UI manifest cleared successfully. System will restart from scratch if files are deleted.",
      );
    } catch (error) {
      console.error(
