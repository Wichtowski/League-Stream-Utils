...
export const isAlmostSquare = (width: number, height: number, tolerance: number = LOGO_SQUARE_TOLERANCE): boolean => {
  ...
  console.log(
    `Square check - Width: ${width}, Height: ${height}, Ratio: ${ratio.toFixed(3)}, Deviation: ${deviation.toFixed(3)}, Tolerance: ${tolerance}, IsSquare: ${isSquare}`
  );
...
const getImageDataFromImage = (img: HTMLImageElement, maxSize: number = 128): ImageData => {
  try {
    ...
    if (!ctx) {
      throw new Error("Canvas context unavailable - 2D context could not be created");
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to get image data from canvas: ${canvasError instanceof Error ? canvasError.message : "Unknown canvas error"}`
      );
    }
  } catch (error) {
...
const labToRgb = (lVal: number, aVal: number, bVal: number): { r: number; g: number; b: number } => {
  ...
  const bLinear = xXyz * 0.0557 + yXyz * -0.204 + zXyz * 1.057;
...
const kMeansClustering = (
  colors: Array<{ lab: { l: number; a: number; b: number }; rgb: { r: number; g: number; b: number }; weight: number }>,
  k: number,
  maxIterations: number = 100
): Array<{ center: { l: number; a: number; b: number }; rgb: { r: number; g: number; b: number }; weight: number }> => {
...
const extractPaletteFromImageData = (imageData: ImageData, maxColors: number = 8): string[] => {
  ...
  const colorMap = new Map<
    string,
    { lab: { l: number; a: number; b: number }; rgb: { r: number; g: number; b: number }; weight: number }
  >();
...
export const extractTeamColorsFromImage = async (src: string): Promise<string[]> => {
  try {
    // If it's an external URL, use the proxy to avoid CORS issues
    let imageSrc = src;
    if (!src.startsWith("data:") && !src.startsWith("/")) {
      const proxyUrl = `/api/v1/assets/proxy?url=${encodeURIComponent(src)}`;
      imageSrc = proxyUrl;
    }
    ...
    const distinct = pickDistinctColors(palette, 8, 0.3);

    // Return all 8 colors for the palette
    return distinct.length >= 8
      ? distinct
      : ["#1d4ed8", "#dc2626", "#ffd700", "#10b981", "#8b5cf6", "#f97316", "#06b6d4", "#84cc16"];
  } catch (error) {
