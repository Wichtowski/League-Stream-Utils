...
class ItemCacheService extends BaseCacheService<ItemCacheData> {
  ...
  // Cache for items.json data to avoid redundant API calls
  private static itemsDataCache: Map<string, DataDragonItemResponse> = new Map();

  /**
   * Fetch and cache items.json data for a specific version
   */
  private async getItemsData(version: string): Promise<DataDragonItemResponse> {
    // Check if we already have this version cached
    if (ItemCacheService.itemsDataCache.has(version)) {
      return ItemCacheService.itemsDataCache.get(version)!;
    }

    // Fetch from DataDragon API
    const response = await fetch(`${DDRAGON_CDN}/${version}/data/en_US/item.json`);
    if (!response.ok) {
      throw new Error(`Failed to fetch items data for version ${version}: ${response.status}`);
    }
  ...
  /**
   * Clear the items data cache (useful when version changes)
   */
  static clearItemsCache(): void {
    ItemCacheService.itemsDataCache.clear();
  }
  ...
  async downloadItemData(itemId: string, version: string): Promise<ItemCacheData> {
    ...
    // Get cached item data (fetches once and caches for all items)
    const itemData = await this.getItemsData(version);
  ...
  private async downloadItemImage(
    itemId: string,
    version: string,
    itemDir: string
  ): Promise<{
    ...
    try {
      const imageResult = await this.downloadAsset(imageUrl, "assets", imageAssetKey);
      if (imageResult) {
        iconPath = `assets/${imageAssetKey}`;
  ...
  async getAllItems(): Promise<ItemCacheData[]> {
    try {
      ...
      // Get cached items data (fetches once and caches for all items)
      const data = await this.getItemsData(version);
  ...
  async downloadAllItemsOnStartup(): Promise<void> {
    try {
      ...
      // Only emit 'downloading' stage if there are items to download
      if (missingItems.length > 0) {
        this.updateProgress({
          current: downloadedCount,
          total: totalItems,
          stage: "downloading",
          itemName: `Starting download of ${missingItems.length} items...`,
      ...
      for (let i = 0; i < missingItems.length; i++) {
        ...
        try {
          ...
          await this.updateCategoryProgress("items", version, itemKey, totalItems, currentDownloadedCount, [
  ...
  private async downloadItemAssets(data: DataDragonItemResponse, version: string): Promise<void> {
    ...
    const downloadPromises = items
      .filter(([_, item]) => item.image?.full)
      .map(async ([_, item]) => {
        ...
        try {
          ...
          console.warn(`Failed to download item ${item.name}:`, error);
        }
      });
  ...
  async getDownloadProgress(version: string): Promise<{ downloaded: number; total: number; percentage: number }> {
    try {
      const data = await this.fetchJson<DataDragonItemResponse>(
        `${DDRAGON_CDN}/${version}/data/en_US/${this.config.endpoint}`
      );
      ...
      for (const [_itemKey, item] of Object.entries(data.data)) {
        if (item.image && item.image.full) {
          const imageKey = `${version}/items/${item.image.full}`;
          // Use inherited checkFileExists method
          if (await this.checkFileExists(imageKey)) {
            downloadedCount++;
          }
        }
      }
...
export const itemsBlueprintDownloader = itemCacheService;
