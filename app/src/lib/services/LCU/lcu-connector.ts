...
class LCUConnector {
  ...
  public setOnChampSelectUpdate(
    handler: (data: ChampSelectSession | null) => void,
  ): void {
  ...
  private async findLCUCredentials(): Promise<LCUCredentials | null> {
    try {
      const response = await fetch(
        "/api/v1/pickban/leagueclient/lcu-credentials",
      );
  ...
  public async connect(): Promise<void> {
    ...
    try {
      ...
      if (!testResult.success) {
        console.warn(
          testResult.message || testResult.error || "LCU test failed",
        );
      }
