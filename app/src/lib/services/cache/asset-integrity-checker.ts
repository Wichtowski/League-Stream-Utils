...
class AssetIntegrityChecker {
  ...
  private async checkChampionAssets(): Promise<{
    ...
    try {
      ...
      const allChampionKeys = championCompleteness.missingChampions.concat(
        Object.keys(championCompleteness).filter(
          (key) => !championCompleteness.missingChampions.includes(key),
        ),
      );
  ...
  private async checkAsset(
    assetKey: string,
  ): Promise<"valid" | "missing" | "corrupted"> {
    ...
    try {
      ...
      if (!fileExists.success) {
        ...
        if (fileExists.error?.includes("File is currently being accessed")) {
          console.warn(
            `File ${manifestEntry.path} is being accessed by another process, skipping...`,
          );
      ...
      if (!sizeResult.success) {
        ...
        if (sizeResult.error?.includes("File access denied")) {
          console.warn(
            `File ${manifestEntry.path} access denied, marking as corrupted`,
          );
          return "corrupted";
        }
        return "corrupted";
  ...
  private extractAssetKeyFromPath(filePath: string): string | null {
    try {
      ...
      const relativePath = filePath.substring(
        cacheIndex + "assets\\cache\\".length,
      );
