...
export abstract class BaseCacheService<T = unknown> {
  ...
  protected async saveCategoryManifest(
    ...
    try {
      ...
      const manifestPath = version
        ? `cache/game/${version}/${category}-manifest.json`
        : `${category}-manifest.json`;
      const manifestContent = JSON.stringify(manifest, null, 2);

      // Use the new category-specific IPC handler, passing the complete path
      const categoryPath = version
        ? `cache/game/${version}/${category}`
        : category;
      const result = await window.electronAPI.saveCategoryManifest(
        categoryPath,
        {
          [manifestPath]: {
  ...
  protected async loadCategoryManifest(
    category: string,
    version?: string,
    ...
    try {
      ...
      const categoryPath = version
        ...
        : category;
      const result =
        await window.electronAPI.loadCategoryManifest(categoryPath);
