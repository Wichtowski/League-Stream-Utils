...
class ItemCacheService extends BaseCacheService<ItemCacheData> {
  ...
  async downloadItemData(
    itemId: string,
    version: string,
  ): Promise<ItemCacheData> {
    ...
    const response = await fetch(
      `${DDRAGON_CDN}/${version}/data/en_US/item.json`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch item data for ${itemId}: ${response.status}`,
      );
    }
    ...
    try {
      ...
      if (typeof window !== "undefined" && window.electronAPI) {
        ...
        for (const itemKey of allItemKeys) {
          ...
          const fileCheck =
            await window.electronAPI.checkFileExists(itemImagePath);
  ...
  async downloadAllItemsOnStartup(): Promise<void> {
    try {
      ...
      console.log(
        `Found ${downloadedCount} items already downloaded out of ${totalItems} total`,
      );
      ...
      this.updateProgress({
        ...
        stage: "downloading",
        itemName:
          missingItems.length > 0 ? `Starting download...` : "All items cached",
        ...
        try {
          ...
          await this.updateCategoryProgress(
            ...
            [
              ...allItemKeys.filter(
                (key) =>
                  !missingItems.includes(key) || missingItems.indexOf(key) <= i,
              ),
            ],
          );
      ...
      console.log(
        "All items downloaded successfully. Cleaning up manifests for fresh restart capability.",
      );
