...
export class AssetDownloader {
  ...
  private updateProgress(progress: Partial<DownloadProgress>): void {
    ...
    const fullProgress: DownloadProgress = {
      ...
      message:
        progress.message ||
        `Downloading ${this.downloadStats.downloadedFiles}/${this.downloadStats.totalFiles} files`,
      errors:
        this.downloadStats.failedFiles > 0
          ? [`${this.downloadStats.failedFiles} files failed`]
          : [],
  ...
  private calculateEstimatedTime(): number {
    ...
    const remainingSize =
      this.downloadStats.totalSize - this.downloadStats.downloadedSize;
  ...
  private updateConnectionStats(): void {
    ...
    this.downloadStats.averageConnections =
      this.totalConnections / this.connectionSamples;
  }
  ...
  async downloadAllAssets(): Promise<{
    ...
    try {
      ...
      const generalWorkers = this.createWorkerPool(
        "general",
      ...
      for (const task of this.downloadQueue) {
        const worker = this.getWorkerForTask(
          task,
          championWorkers,
          itemWorkers,
          generalWorkers,
  ...
  private async buildDownloadQueueFromIntegrity(
    integrityResult: AssetIntegrityResult,
  ): Promise<void> {
    this.downloadQueue = [];

    const allMissingAssets = [
      ...integrityResult.missingAssets,
      ...integrityResult.corruptedAssets,
    ];

    // Group assets by type for better organization
    const championAssets = allMissingAssets.filter((asset) =>
      asset.includes("/champions/"),
    );
    const itemAssets = allMissingAssets.filter((asset) =>
      asset.includes("/item/"),
    );
    const spellAssets = allMissingAssets.filter((asset) =>
      asset.includes("/spell/"),
    );
    const runeAssets = allMissingAssets.filter((asset) =>
      asset.includes("/rune/"),
    );
    ...
    if (championMatch) {
      ...
      if (assetKey.endsWith("/data.json")) {
        ...
        if (abilityMatch) {
          ...
          const abilityKey =
            ability === "passive" ? "Passive" : ability.toUpperCase();
  ...
  private addChampionTasks(championKey: string): void {
    ...
    abilities.forEach((ability) => {
      const fileName =
        ability === "passive" ? "passive.png" : `${ability.toLowerCase()}.png`;
      this.downloadQueue.push({
      ...
      try {
        ...
        if (typeof window !== "undefined" && window.electronAPI) {
          ...
          if (result.success) {
            ...
            this.downloadStats.downloadedSize +=
              result.size || task.size || 50000;
        ...
        if (attempt >= maxRetries) {
          ...
          console.warn(
            `Failed to download ${task.id} after ${maxRetries} attempts:`,
            error,
          );
        } else {
  ...
  private getAssetDisplayName(assetKey: string): string {
    ...
    if (assetKey.includes("/champions/")) {
      ...
      if (championMatch) {
        ...
        if (fileName === "splashCentered.jpg")
          return `${championName} Centered`;
