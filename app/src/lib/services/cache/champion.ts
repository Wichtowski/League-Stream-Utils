...
class ChampionCacheService extends BaseCacheService<Champion> {
  ...
  async downloadChampionData(
    championKey: string,
    version: string,
  ): Promise<ChampionCacheData> {
    ...
    const manifestResult =
      await window.electronAPI.loadCategoryManifest("champions");
    ...
    const response = await fetch(
      `${DDRAGON_CDN}/${version}/data/en_US/champion/${championKey}.json`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch champion data for ${championKey}: ${response.status}`,
      );
    }

    const championData: { data: { [key: string]: DataDragonChampion } } =
      await response.json();
    ...
    const images = await this.downloadChampionImages(
      championKey,
      version,
      championDir,
    );
    ...
    const spells = await this.downloadAbilityImages(
      champion,
      version,
      championDir,
    );
  ...
  private async downloadChampionImages(
    ...
    const downloadPromises = Object.entries(imageUrls).map(
      async ([key, url]) => {
        ...
        try {
          if (typeof window !== "undefined" && window.electronAPI) {
            ...
            if (downloadResult.success) {
              ...
              if (key === "splashCentered" || key === "loading") {
                ...
                const fallbackResult = await window.electronAPI.downloadAsset(
                  fallbackUrl,
                  "cache",
                  assetKey,
                );
                ...
                console.warn(
                  `Failed to download ${key} image for ${championKey}:`,
                  downloadResult.error,
                );
          ...
          console.warn(
            `Error downloading ${key} image for ${championKey}:`,
            _error,
          );
          // For splashCentered and loading, fallback to regular splash
          if (key === "splashCentered" || key === "loading") {
      ...
      downloadPromises.push(
        (async () => {
          if (typeof window !== "undefined" && window.electronAPI) {
            ...
            const baseDownloadResult = await window.electronAPI.downloadAsset(
              baseImageUrl,
              "cache",
              baseAssetKey,
            );
            if (baseDownloadResult.success) {
              ...
              console.warn(
                `Failed to download base spell image for ${spell.id}:`,
                baseDownloadResult.error,
              );
              spells.push({
                spellName: spell.id,
        ...
        downloadPromises.push(
          (async () => {
            if (typeof window !== "undefined" && window.electronAPI) {
              ...
              const recastDownloadResult =
                await window.electronAPI.downloadAsset(
                  recastImageUrl,
                  "cache",
                  recastAssetKey,
                );
      ...
      (async () => {
        if (typeof window !== "undefined" && window.electronAPI) {
          ...
          if (passiveDownloadResult.success) {
            ...
            console.warn(
              `Failed to download passive image for ${champion.id}:`,
              passiveDownloadResult.error,
            );
            spells.push({
              spellName: `${champion.id}Passive`,
  ...
  private getRecastSpellVariants(
    championId: string,
    spellId: string,
  ): Array<{ suffix: string; imageName: string }> {
  ...
  private async cleanupManifestAfterSuccess(): Promise<void> {
    try {
      ...
      console.error(
        "Failed to cleanup manifest after successful download:",
        error,
      );
    }
  }
  ...
  async checkCacheCompleteness(): Promise<{
    ...
    try {
      ...
      console.log(
        `Checking file existence for ${allChampionKeys.length} champions...`,
      );
  ...
  private async migrateExistingChampions(
    ...
    try {
      ...
      if (existingChampions.length > 0) {
        console.log(
          `Found ${existingChampions.length} champions in current version ${version}`,
        );
      } else {
        // If no champions found in current version, check previous versions
        console.log(
          `No champions found in version ${version}, checking previous versions...`,
        );
      ...
      if (existingChampions.length > 0) {
        ...
        console.log(
          `Successfully migrated ${existingChampions.length} existing champions to category manifest`,
        );
      } else {
  ...
  async downloadAllChampionsOnStartup(): Promise<{
    ...
    try {
      ...
      console.log(
        `Starting automatic champion download for version ${version}...`,
      );
      ...
      if (downloadedCount > 0) {
        console.log(
          "All champions downloaded successfully. Cleaning up manifests for fresh restart capability.",
        );
