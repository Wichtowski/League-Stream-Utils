            ...
            <div key={player.id} className="bg-gray-700 rounded p-3 text-center">
              ...
              {player.verified ? (
                <div className="text-xs text-green-400">
                  ✓ Verified
                  {player.verifiedAt && (
                    <div className="text-gray-500">{new Date(player.verifiedAt).toLocaleDateString()}</div>
                  )}
                </div>
              ) : (
                <button
      ...
      <div className="mt-4 p-3 bg-gray-700 rounded">
        ...
        <div className="text-xs text-gray-400 mt-1">
          ...
          {team.players.substitutes.length > 0 && (
            <span>
              {" "}
              • Subs: {team.players.substitutes.filter((p) => p.verified).length}/{team.players.substitutes.length}{" "}
              verified
            </span>
          )}
        </div>
