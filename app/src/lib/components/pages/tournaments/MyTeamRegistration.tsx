...
export const MyTeamRegistration = ({
  ...
  const availableTeams = filteredTeams.filter(
    (team) => !tournament.registeredTeams.includes(team.id),
  );
  const registeredTeams = filteredTeams.filter((team) =>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-400 mt-1">
                Register your teams to {tournament.name}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          {myTeams.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">
                You need to create a team first before registering for
                tournaments.
              </p>
            </div>
          ) : (
            ...
            <div>
              ...
              {team.players.substitutes.length > 0 && (
                <p className="text-gray-300">
                  {team.players.substitutes.length} subs
                </p>
              )}
            </div>
            <div>
              ...
              <div className="space-y-1">
                ...
                {status.hasUnverifiedPlayers && (
                  <p className="text-amber-400 text-xs">
                    ⚠️ {status.unverifiedPlayers.length} unverified
                  </p>
                )}
                {status.canRegister && (
          ...
          {!status.canRegister && !isRegistered && (
            <div className="mt-3 p-2 bg-amber-600/20 border border-amber-600/40 rounded text-xs text-amber-300">
              ...
              <ul className="list-disc list-inside mt-1 space-y-1">
                {!status.hasCompleteRoster && (
                  <li>Complete 5-player roster required</li>
                )}
                {status.hasUnverifiedPlayers && (
                  <li>All players must be verified</li>
                )}
              </ul>
            </div>
          )}
        ...
        <div className="ml-4">
          {isRegistered ? (
            ...
            <button
              ...
              disabled={
                isLoading ||
                (!status.canRegister && tournament.status === "registration")
              }
