...
export default function SponsorManager({ tournamentId, tournament: _tournament, onSponsorsUpdated }: SponsorManagerProps) {
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={formData.tier}
                onChange={(e) => setFormData(prev => ({ ...prev, tier: e.target.value as Sponsorship['tier'] }))}
                className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2"
              >
            ...
            <div key={sponsor.id} className="bg-gray-800 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  sponsor.tier === 'title' ? 'bg-yellow-600 text-yellow-100' :
                  sponsor.tier === 'presenting' ? 'bg-purple-600 text-purple-100' :
                  sponsor.tier === 'official' ? 'bg-blue-600 text-blue-100' :
