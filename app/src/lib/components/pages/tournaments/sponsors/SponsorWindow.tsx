...
export const SponsorWindow = ({
  ...
  if (!currentSponsor) {
    return (
      <div
        className={`${fixed ? "fixed bottom-4 left-4" : ""} w-64 h-32 bg-black bg-opacity-50 rounded-lg`}
      >
  ...
  return (
    <div
      className={`${fixed ? "fixed bottom-0 left-0" : ""} w-78 h-39 bg-black bg-opacity-50`}
    >
      <div
      ...
      >
        <div className="flex items-center gap-2">
          {currentSponsor.showName &&
            currentSponsor.namePosition === "left" && (
          ...
          {currentSponsor.showName &&
            currentSponsor.namePosition === "right" && (
              <span className="text-white text-sm font-semibold whitespace-nowrap">
                {currentSponsor.name}
              </span>
            )}
