...
export const AdminTournamentManager = ({
  onClose,
}: AdminTournamentManagerProps): React.ReactElement => {
  ...
  const handleRegisterTeam = useCallback(async (): Promise<void> => {
    if (!selectedTournament || !selectedTeam) {
      await showAlert({
        type: "error",
        message: "Please select both a tournament and a team",
      });
      return;
    }
  ...
  const handleUnregisterTeam = useCallback(
    async (tournamentId: string, teamId: string): Promise<void> => {
      try {
        const response = await fetch("/api/v1/admin/tournaments/register", {
          method: "DELETE",
          headers: {
      ...
      <div className="bg-gray-800 rounded-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-700">
          ...
          <p className="text-gray-400 mt-2">
            Register any team to any tournament with admin privileges
          </p>
        </div>
        ...
        <div className="p-6">
          ...
          {selectedTournament && selectedTeam && (
            <div className="mt-6 p-4 bg-gray-700 rounded-lg">
              ...
              <div className="text-xs text-gray-400 space-y-1">
                <p>
                  🔧 <strong>Admin Bypass:</strong> This registration will
                  ignore:
                </p>
          ...
          {selectedTournamentData &&
            selectedTournamentData.registeredTeams.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-white mb-4">
                  Registered Teams (
                  {selectedTournamentData.registeredTeams.length})
                </h3>
