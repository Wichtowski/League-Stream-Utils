...
const PlayerSlotComponent: React.FC<PlayerSlotProps> = ({
  player,
  index,
  teamColor,
  _currentPhase,
  ...
  const isCurrentlyPicking =
    hoverState?.currentActionType === "pick" && isCurrentPickingPlayer;
      ...
      >
        {!isPlaceholder && image && image.trim() !== "" ? (
          ...
          <div className="absolute inset-0 w-full h-full bg-transparent flex items-center justify-center text-gray-500 text-sm z-0">
            {!isPlaceholder &&
            player.role &&
            PLAYER_CARD_ROLE_ICONS[player.role] ? (
              <Image
              ...
              <div className="text-center">
                ...
                <div className="text-sm">
                  {isPlaceholder ? "Empty Slot" : "No Role"}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="relative z-20 p-3 flex flex-col">
          ...
          <div
            className="text-xs"
            style={{ color: isPlaceholder ? "#9CA3AF" : `${teamColor}CC` }}
          >
            {isPlaceholder
              ? "No Champion"
