...
export const OBSControl = () => {
    ...
    const [streamingStatus, setStreamingStatus] = useState<OBSStreamingStatus>({ isStreaming: false, isRecording: false });
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'success', message: 'Connected to OBS successfully!' });
            } else {
                await showAlert({ type: 'error', message: `Failed to connect to OBS: ${result.error}` });
            }
        } catch (error) {
    ...
    const handleSetScene = async (sceneName: string) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'success', message: `Switched to scene: ${sceneName}` });
            } else {
                await showAlert({ type: 'error', message: `Failed to switch scene: ${result.error}` });
            }
        } catch (error) {
    ...
    const handleStartStreaming = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'error', message: `Failed to start streaming: ${result.error}` });
            }
        } catch (error) {
    ...
    const handleStopStreaming = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'error', message: `Failed to stop streaming: ${result.error}` });
            }
        } catch (error) {
    ...
    const handleStartRecording = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'error', message: `Failed to start recording: ${result.error}` });
            }
        } catch (error) {
    ...
    }, [isElectron, loadScenes, loadSources, loadCurrentScene, loadStreamingStatus]);
    ...
    const handleStopRecording = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await showAlert({ type: 'error', message: `Failed to stop recording: ${result.error}` });
            }
        } catch (error) {
        ...
        <div className="space-y-6">
                ...
                {!isConnected ? (
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <input
                                    type="number"
                                    value={config.port}
                                    onChange={(e) => setConfig(prev => ({ ...prev, port: parseInt(e.target.value) || 4455 }))}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="4455"
                                />
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {isConnected && (
                            <div className="bg-gray-800 shadow rounded-lg p-6">
                                <h3 className="text-lg font-medium text-gray-100 mb-4">Scene Control</h3>
                                
                                <div className="mb-4">
                                    <p className="text-sm text-gray-300 mb-2">
                                        Current Scene: <span className="font-medium text-gray-100">{currentScene || 'None'}</span>
                                    </p>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {scenes.map((scene) => (
                            ...
                            >
                                <div className="font-medium">{scene.sceneName}</div>
                                                                            <div className="text-sm text-gray-400">Scene {scene.sceneIndex}</div>
                            </button>
                        ))}
                    </div>
                </div>
                        ...
                        {isConnected && (
                            <div className="bg-gray-800 shadow rounded-lg p-6">
                                <h3 className="text-lg font-medium text-gray-100 mb-4">Stream & Recording Control</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Streaming */}
                                        ...
                                        <div className="flex items-center gap-2 mb-3">
                                            <div className={`w-3 h-3 rounded-full ${streamingStatus.isStreaming ? 'bg-red-500' : 'bg-gray-300'}`}></div>
                        ...
                        {/* Recording */}
                                        ...
                                        <div className="flex items-center gap-2 mb-3">
                                            <div className={`w-3 h-3 rounded-full ${streamingStatus.isRecording ? 'bg-red-500' : 'bg-gray-300'}`}></div>
                                            <span className="text-sm text-gray-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {sources.map((source) => (
                                                            <div key={source.sourceName} className="p-3 border border-gray-600 rounded-lg bg-gray-700">
                                    <div className="font-medium text-sm text-gray-100">{source.sourceName}</div>
