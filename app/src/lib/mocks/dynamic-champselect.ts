...
export class DynamicChampSelectMock {
  ...
  public setTurn(turn: number): void {
    this.config.currentTurn = Math.max(
      0,
      Math.min(turn, PICK_BAN_ORDER.length - 1),
    );
    this.updatePhase();
  ...
  public setPhase(phase: MockPhase): void {
    ...
    const phaseStartTurn = PICK_BAN_ORDER.findIndex(
  ...
  public getSelectionState(): {
    ...
    return {
      isHovering,
      isSelecting,
      hoveredChampionId: isHovering
        ? this.getChampionForCycle(championCycle)
        : null,
    };
  }
  ...
  public generateMockData(): EnhancedChampSelectSession {
    ...
    if (timeLeft <= 0 && !isInfinite) {
      ...
      const newBaseTime =
        this.config.currentPhase === "FINALIZATION" ? 59000 : 27000;
      this.config.timerStartTime = Date.now();
      elapsed = 0;
      timeLeft = newBaseTime;
    }
    // Calculate bans and picks
    const blueBans = completedActions
      .filter((a) => a.team === "blue" && a.type === "ban")
      .map((a) => a.championId);
    const redBans = completedActions
      .filter((a) => a.team === "red" && a.type === "ban")
      .map((a) => a.championId);
    const bluePicks = completedActions
      .filter((a) => a.team === "blue" && a.type === "pick")
      .map((a) => a.championId);
    const redPicks = completedActions
      .filter((a) => a.team === "red" && a.type === "pick")
      .map((a) => a.championId);
