...
export const useTurnSequence = (
  ...
  useEffect(() => {
    ...
    const currentAction =
      currentTurn < PICK_BAN_ORDER.length ? PICK_BAN_ORDER[currentTurn] : null;
    const nextAction =
      currentTurn + 1 < PICK_BAN_ORDER.length
        ? PICK_BAN_ORDER[currentTurn + 1]
        : null;
...
export const useChampionHoverAnimation = (
  ...
  const [hoveredChampionId, setHoveredChampionId] = useState<number | null>(
    null,
  );
  ...
  const getAnimationClasses = (championId: number): string => {
    ...
    if (isCurrentTurn && currentTeam) {
      const isHoveredByCurrentTeam =
        (currentTeam === "blue" && isHoveredByBlue) ||
        (currentTeam === "red" && isHoveredByRed);
