...
export async function createTeam(
  ...
  const substitutePlayers: Player[] = teamData.players.substitutes.map(
    (player) => ({
      ...player,
      id: uuidv4(),
      verified: false,
...
export async function verifyTeamPlayers(
  ...
  playerUpdates.forEach((update) => {
    // Check main players
    const mainPlayer = team.players.main.find(
      (p: Player) => p.id === update.playerId,
    );
    if (mainPlayer) {
    ...
    const subPlayer = team.players.substitutes.find(
      (p: Player) => p.id === update.playerId,
    );
    if (subPlayer) {
  ...
  const allMainVerified =
    team.players.main.length > 0 &&
    team.players.main.every((p: Player) => p.verified);
  const allSubsVerified =
    team.players.substitutes.length === 0 ||
    team.players.substitutes.every((p: Player) => p.verified);
