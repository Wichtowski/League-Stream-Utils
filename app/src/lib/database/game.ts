...
export async function createGameSession(
  sessionData: Partial<GameSessionType>,
...
export async function getAllGameSessions(): Promise<GameSessionType[]> {
  ...
  try {
    ...
    return sessions.map((doc) =>
      transformToGameSession(doc as MongooseDocument),
    );
  } catch (error) {
...
export async function getUsedChampionsInSeries(
  sessionId: string,
): Promise<Champion[]> {
  await connectToDatabase();
  ...
  try {
    ...
    const transformedSession = transformToGameSession(
      session as MongooseDocument,
    );
...
export async function addUsedChampion(
  sessionId: string,
  teamSide: "blue" | "red",
  champion: Champion,
): Promise<void> {
