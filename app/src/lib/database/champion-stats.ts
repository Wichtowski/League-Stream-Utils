...
const TournamentChampionStatsModel =
  models.TournamentChampionStats ||
...
export async function updateChampionStats(
  tournamentId: string,
  ...
  try {
    ...
    const allPicks = [
      ...gameResult.blueTeam.picks,
      ...gameResult.redTeam.picks,
    ];
    ...
    for (const pick of allPicks) {
      await updateChampionStat(tournamentStats, pick.championId, "pick", {
        ...
        won:
          allPicks.indexOf(pick) < 5
            ? gameResult.blueTeam.won
            : gameResult.redTeam.won,
        role: pick.role,
      });
    }
    ...
    for (const champStat of tournamentStats.championStats) {
      champStat.pickRate =
        (champStat.totalPicks / tournamentStats.totalGames) * 100;
      champStat.banRate =
        (champStat.totalBans / tournamentStats.totalGames) * 100;
      champStat.presenceRate =
        ((champStat.totalPicks + champStat.totalBans) /
...
async function updateChampionStat(
  ...
  let champStat = tournamentStats.championStats.find(
    (stat: ChampionStats) => stat.championId === championId,
  );
...
export async function getTournamentChampionStats(
  tournamentId: string,
): Promise<TournamentChampionStats | null> {
  await connectToDatabase();
  ...
  try {
    ...
    const topPicks = [...championStats]
      .sort((a, b) => b.totalPicks - a.totalPicks)
      .slice(0, 10);

    const topBans = [...championStats]
      .sort((a, b) => b.totalBans - a.totalBans)
      .slice(0, 10);

    const topPresence = [...championStats]
    ...
    const blueSidePriority = [...championStats]
      ...
      .sort(
        (a, b) =>
          b.blueSidePicks / Math.max(b.totalPicks, 1) -
          a.blueSidePicks / Math.max(a.totalPicks, 1),
      )
      .slice(0, 10);
    ...
    const redSidePriority = [...championStats]
      ...
      .sort(
        (a, b) =>
          b.redSidePicks / Math.max(b.totalPicks, 1) -
          a.redSidePicks / Math.max(a.totalPicks, 1),
      )
      .slice(0, 10);
...
export async function getChampionStatsForOBS(tournamentId: string): Promise<{
  ...
  return {
    ...
    topPicks: stats.topPicks.slice(0, 5).map((stat) => ({
      ...
      winRate:
        stat.totalPicks > 0
          ? Math.round((stat.wins / (stat.wins + stat.losses)) * 1000) / 10
          : 0,
    })),
