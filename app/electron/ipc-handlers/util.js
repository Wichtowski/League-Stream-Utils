...
function registerUtilHandlers() {
    ...
    ipcMain.handle('get-file-size', async (_event, filePath) => {
        if (!acquireLock(filePath)) {
            return { success: false, error: 'File is currently being accessed by another process' };
        }
        ...
        try {
            ...
            if (error.code === 'EPERM') {
                return { success: false, error: 'File access denied - file may be in use by another process' };
            }
            return { success: false, error: error.message };
        } finally {
    ...
    ipcMain.handle('clear-asset-cache', async () => {
        try {
            ...
            if (isModeSwitching) {
                ...
                return { success: false, error: 'Cache clearing blocked during mode switching' };
            }
