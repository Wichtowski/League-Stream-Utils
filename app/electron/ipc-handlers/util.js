      ...
      if (fs.existsSync(assetCachePath)) {
        ...
        try {
          ...
          if (error.code === "EPERM" || error.code === "EBUSY") {
            console.warn(
              "Recursive cache removal failed, attempting individual file removal...",
            );
          ...
          try {
            ...
            for (const item of items) {
              ...
              try {
                ...
                if (error.code === "EPERM") {
                  console.warn(
                    `Skipping file ${fullPath} due to access error:`,
                    error.message,
                  );
                  corruptedFiles.push(
                    `${category}/${path.join(relativePath, item)}`,
                  );
                } else {
            ...
            if (error.code === "EPERM") {
              console.warn(
                `Skipping directory ${dirPath} due to access error:`,
                error.message,
              );
            } else {
